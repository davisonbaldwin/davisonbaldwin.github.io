/* Gen 6 body: the generation where the PACKAGE moved.
   ...
   Five generations optimised Cd against a frontal area nobody was allowed to
   ...
   is proportional to CdA, not Cd, so the ladder had been optimising the
   ...
   stations, integrated in y, plus the lower nose band and the tyre columns):
   ...
   brief sized a
   front fairing against the steering sweep at hub height, where the tyre's
   outer corner reaches |z| 0.9856 at 14 degrees of lock. That is not the
   binding corner. A 185-section tyre is full width to within 30 mm of its
   ...
   bonnet skin passes through), hv-4 (48 V zonal actuation), suspension-4
   (+-3 degree rear steer, ride-height hold), autonomy-4 (lidar pucks at
   (2.33, 0.615, +-0.30), camera pods), wheels-6 (185/50 R21, static tyre
...
export const SYSTEM = {
  ...
  parts: {
    ...
    'front-casting': {
      ...
      how: 'The casting is unchanged from the Gen 1 drawing: molten AlSi10MnMg injected at 700 degrees C under partial vacuum, filling a 1.5 m part in under 100 milliseconds, reaching strength by natural ageing because a casting this large would warp in the water quench a T6 treatment needs. It gathers the crash rails, the upper apron loads and the rocker fronts, and it stays elastic while the bolted rails ahead of it fold. Every suspension and drive mount is machined in one fixture, so the front axle geometry hangs off a single datum, which is the whole reason suspension-4 can hold plus or minus 0.15 mm on its pivot bores.\n\nWhat this generation asked of it is worth stating precisely, because the answer was nothing. Gen 6 lowers the roof crown 92 mm and pulls the beltline in 15 mm, and the casting lives entirely below y 0.72 and inside |z| 0.60. It is under the bonnet, under the frunk, inboard of the new front wheel fairings by more than 400 mm. The finding that opened this generation was that a shared cage froze the roofline; nothing about a shared front casting froze anything, and it would have been dishonest to redraw a part just to make the generation look busier.\n\nOne thing did change and it is a correction rather than a redraw: the cross portion that ties the two spars. It was a solid 0.31 by 0.28 by 0.68 m block filling x 1.760 to 2.070 over the casting\'s full section height, 0.0590 cubic metres of drawn aluminium against 48 kg declared for the whole part, and it stood in the volume thermal-6 uses for its heat pump, glycol loop and pressure set. Measured by ray parity, thermal-6/pressure-set was 93.0 percent enclosed by this casting. It is now a lower crossmember at the casting front face, x 2.000 to 2.070 and y 0.300 to 0.370, and the bay above it is open: pressure-set falls from 60.0 to 45.0 mm, thermal-6/radiator from 48.0 to 33.0, thermal-6/heat-pump from 20.0 to 7.3, and thermal-6/glycol-loop clears entirely. Every hardpoint is where it was.',
      why: 'The discipline of a ladder is knowing which inherited part is a constraint and which is merely inherited. The cage was a constraint: it set the roof, and through the roof the frontal area, and through the frontal area 40 percent of motorway drag. The front casting sets the front axle datum and nothing above y 0.72. Redrawing it would have moved 48 kg of proven hardware to buy zero counts and would have put every front suspension pickup back on the table, which is the trade an unconstrained programme finds it easiest to get wrong: change is not the same thing as improvement, and a generation that renumbers a part it did not improve has spent its credibility rather than its resources.',
      fail: [
        ...
        'Every steel bolt into aluminium is a galvanic couple; the new cage is hot-stamped steel and lands on this casting through the same adhesive-and-rivet isolation the Gen 1 cage used.',
      ],
      explode: [0.85, -0.35, 0],
    },
    'rear-casting': {
      ...
      fail: [
        ...
        'Yield the casting and the shell is done, unchanged from Gen 1, and now with a longer tail to re-index afterwards.',
      ],
      explode: [-0.85, -0.35, 0],
    },
    'crash-rails': {
      ...
      specs: [
        ...
        ['Rail centreline', '|z| 0.550, was 0.420, flush with the spar wall'],
    ...
    cage: {
      ...
      fail: [
        ...
        'The rail inner face sits at |z| 0.482, a 15 mm crushable FMVSS 201 head pad takes it to 0.467, and the 95th percentile head shadow reaches 0.46: seven millimetres of air, which is why that pad is a certified component rather than trim and why its material is on the homologation drawing.',
    ...
    skin: {
      ...
      fail: [
        'Thin-ply carbon takes barely visible impact damage: hail or a car-park knock can delaminate plies under paint that looks perfect, so ultrasound spot checks stay on the service schedule, carried from body-6.',
        'The bonnet leading edge now runs at 43 degrees where body-6\'s ran at 22, because thermal-6\'s condenser inlet header tops out at y 0.770 at x 2.166 to 2.224 and the skin has to pass above it; that reshapes the pedestrian wrap-around zones and the head-impact map has to be re-run rather than carried.',
    ...
    'front-fairings': {
      ...
      fail: [
        ...
        'Snow chains are geometrically impossible inside a closed fairing, and unlike body-6\'s crescents there is no quick-release panel that restores them; the answer is winter tyres and the manual has to say so.',
        'A punctured front tyre cannot be inspected without removing the fairing lower closeout, four fasteners and a bonded seal, so the roadside case is a tyre-pressure sensor and a tow rather than a visual check.',
        'The fairing is designed frangible outboard of the wheel so that a kerb strike tears it free instead of levering it into the tyre at lock; that is the correct failure but it means a parking scuff can write off a bonded structural pod rather than scratching a fender.',
      ],
    ...
    'tail-cone': {
      ...
      fail: [
        ...
        'Boot volume is the visible bill: a tapered tail that reaches 0.550 half-width has no usable width behind the rear axle, so the load floor stops at x -2.15 and this is a two-boot car, frunk and a shallow rear well, not an estate.',
    ...
    canopy: {
      ...
      fail: [
        'One deep chip still condemns the band and the replacement must re-index to the loft within half a millimetre, carried from body-6 unimproved.',
    ...
    'diffuser-flap': {
      ...
      fail: [
        ...
        'Deep snow packs the expansion and the diffuser becomes a plough; rated for the load, parked at 13 by the controller, and still beyond the drag model\'s vocabulary, carried from body-6 with the same apology.',
      ],
      explode: [-1.0, -0.4, 0],
    },
    'arch-louvers': {
      ...
      fail: [
        ...
        'A blade struck through the aperture is frangible by design, so it tears free rather than levering into the tyre, and the current signature logs exactly when it happened.',
      ],
    ...
    'venturi-floor': {
      ...
      fail: [
        'Kerbs and ramps still rake the lip first; the shear-away fasteners cost a bracket, carried through four generations because car parks have not changed.',
        'A narrower splitter feeds a narrower stagnation region into the tunnels, so the front tunnel inlets are more sensitive to yaw than body-6\'s were and the crosswind case is the one the tunnel programme should re-run first.',
...
/* ── Geometry ──────────────────────────────────────────────────────────────
   ...
   - Frontal area, integrated the same way as body-6: silhouette 1.822 above
     y 0.28, lower nose band 0.173, tyre columns 0.104, A 2.099 m2. At the
   ...
   - Beltline half-width 0.925: wheels-6 static tyre face 0.9025 plus skin,
     liner and clearance.
   - ARCH CLEARANCE is audited against the swept SOLID of the tyre, not
     against a single corner value, because a 185-section tyre is full width
   ...
   - Carried condition, present since body.js: the bumper beam's outboard
     ...
     part of it that ever was. Face centres do not move under a chamfer, so
   ...
   the old car had two 15 mm slots between the front casting's centre box
   ...
   clearance in the list above is quoted to the millimetre off the built
...
const STATIONS = [
  ...
  /* 3  front fairing, forward sweep peak (x 1.45 + 0.367). The shoulder point
        holds 0.936 to y 0.76 because the swept tyre's UPPER outer corner, not
  ...
  /* 10 rear axle. Haunch shoulder held to y 0.76 for the same reason as the
        fairing: the tyre is full section width to within 30 mm of its crown. */
...
/* Half-width of the loft at an arbitrary (x, y): linear between the two
   ...
   of by luck. That is the body-4 floating-splitter lesson generalised: if a
...
/* y at a fractional half-profile index, so a feature line can follow the
   ...
   windscreen header and 0.832 at the rear door, because the flank's own
...
/* ── The end caps ────────────────────────────────────────────────────────
   ...
   unfilled: a 52 by 14 mm hole dead centre in the nose whose top edge is at
   ...
   centre vertex instead, so there is nothing left to fill.
...
/* Bonnet, x 2.220 to 1.130: forward of the cowl at station 5 where the
   ...
   Boot lid, x -1.890 to -2.320. This body keeps its backlight, so the
...
/* ── The louver recesses ────────────────────────────────────────────────
   ...
   EACH BANK IS CENTRED IN ITS BAY, which moves the aft bank 68 mm forward
...
/* ── The capacitive pad recesses ────────────────────────────────────────
   ...
   calls flush to a quarter of a millimetre. Recessing it is a correction
...
/* Seat a slab of thickness t on a pocket floor cut `depth` into the flank:
   the position of its centre and the two rotations that lay it flat. The
...
/* ── The declared hard lines ────────────────────────────────────────────
   ...
   65 degrees at every station from the windscreen header aft and 10 degrees
   ...
   A LINE IS DECLARED WITH ITS EXTENT. The bonnet side gaps run from ring
...
/* Split normals along the declared lines only. lib.crease cannot do this:
   ...
   is a triangulation artefact showing through the paint.
...
function buildStructure(sys) {
  ...
  /* front crash rails and bumper beam, coordinates carried. The rails bolt
     ...
     them, because the flange is under a closed bonnet. Geometry nobody can
...
function buildShell(sys) {
  /* ── skin: nose flank, the fixed panels either side of the door aperture,
     the bonnet band and the nose face. Every painted panel is a slice of
  ...
  skin.add(panel([RI(0), RI(COWL)], UP));             /* nose deck, bonnet, cowl */
  ...
  for (const s of [-1, 1]) {
    /* inboard wall: 17.5 mm inboard of wheels-6's tyre inner face, which is
  ...
  /* ── tail cone: flank and deck bands aft of the arch, plus the Kamm face.
     Plan half-width 0.948 at x -1.83 down to 0.550 at -2.675. The boot lid
     is cut into the deck band rather than sliced out of it, because it does
     not open in this model and one mesh keeps its channel closed by
     construction. ── */
  const tail = lib.part('tail-cone', [-1.25, 0.30, 0]);
  tail.add(panel([RI(DECK), RI(LAST)], FL));
  tail.add(panel([RI(DECK), RI(LAST)], FR));
  tail.add(panel([RI(DECK), RI(LAST)], UP));
  ...
  /* ── light bands. Both were a single slab standing 14 mm off a coned cap,
     ...
     The front band face stays at x 2.371 to the millimetre and the lidar
  ...
  /* The blade is a SECTION, not a plank. Span, chord, station and deployed
     ...
     the span with a 2 mm bevelled rim, which is what gives the tip an edge
  ...
  /* A tapering deck is not a plane of constant height across a 50 mm foot,
     ...
     the height at its centre buries one corner 7 mm and floats the opposite
  ...
  /* ── diffuser flap, drawn deployed at 17.5 degrees: 0.70 m ramp hinged
     ...
     clear of wheels-6's tyre inner faces at 0.7175 and of drivetrain-6's
  ...
  for (const z of [-0.44, -0.18, 0.18, 0.44]) {
    ...
    /* two fixings per strake, taking the ramp's own rotation so the heads
       sit flat on the panel they bolt through. The strake centre line
  ...
  for (const [, , , xa, xb] of BANK) {
    for (const x of [xa, xb]) {
      ...
      /* one fixing through each frame. A louver bank is a serviceable item
         ...
         true. The seat is the FRAME's own face centre and the axis its own
  ...
  /* The lattice is sized to the aperture rather than to itself: 0.800 by
     ...
     Sixteen millimetre bars with the cross family 12 mm behind them give
  ...
  /* capacitive strips, in the pockets cut for them. A pocket floor is not a
     plane of constant |z|: the flank draws in 3.7 mm over the rear strip's
     own 140 mm, so a straight bar seated at the pocket's mid-station buries
     one end 2.9 mm and stands the other 5.0 mm proud. Each strip is turned
     onto its own floor's line, seated from floorZ, and stands 2 mm proud of
     the door rather than the 14 mm it stood before. The pocket is sized from
     these three numbers, so the strip lands on full-depth floor over its
     whole footprint and both run-outs fall outside it. */
  for (const x of PADX) {
    const y = ringY(x, PADCOL);
    const s = seatOnFlank(x, y, PAD.depth, PADT);
    doors.add(acbox(PADL, PADH, PADT, 0.0025, M.alu, s.p[0], s.p[1], s.p[2], 0, s.rx, s.ry));
  }
  /* camera pods on 90 mm stalks, on the fixed strip ahead of the front
     shutline where the hinge boxes live, not on the door skin */
  const zr = halfWidth(0.98, 0.870);
  doors.add(acbox(0.014, 0.012, 0.09, 0.003, M.plastic, 0.98, 0.870, zr + 0.045));
  doors.add(acbox(0.05, 0.028, 0.028, 0.005, M.sensor, 0.98, 0.870, zr + 0.098));
  sys.add(doors);
