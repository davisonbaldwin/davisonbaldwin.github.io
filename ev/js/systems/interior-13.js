/* Interior, Gen 13: interior-11 with both seats taken off their risers and
...
export const SYSTEM = {
  id: 'interior-13',
  name: 'Interior · Gen 13 the floor',
  color: 0xd9c08a,
  explode: [0, 1.05, 0],
  ...
  parts: {
    ...
    'rear-seat-cushion': {
      ...
      fail: [
        ...
        'The rear occupant\'s posture is a declaration and a declaration can be wrong. tools/occupant.js now takes both the hip and the leg half-separation from SYSTEM.occupants and measures them against the built seat, which is the right shape of check, but it can only measure what is declared: declare the feet at |z| 0.300 and the tool dutifully drives them 73.0 mm through the seat in front, and the number that catches it comes from a foot box swept by hand rather than from the gate. The gate reports sink and intrusion; nothing in this repo checks that a declared leg station is inside the channel the same module cut.',
      ],
      explode: [-0.25, 0.45, 0],
    },
    pedals: {
...
export function build() {
  ...
  const wheelP = lib.part('wheel', [0.45, 0.15, 0]);
  {
    ...
    rim.rotation.x = -0.28;
    rim.position.set(0.680, 0.805, 0);
    wheelP.add(rim);
    /* flat-bottom chord */
    ...
    wheelP.add(abox(0.026, 0.120, 0.120, M.castAlu, 0.556, 0.805, 0));
    ...
    for (const s of [-1, 1]) {
      ...
      wheelP.add(abox(0.070, 0.070, 0.014, M.castAlu, 0.6380, 0.8110, s * 0.0610));
    }
