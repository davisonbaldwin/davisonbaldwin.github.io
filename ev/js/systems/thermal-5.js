...
export const SYSTEM = {
  ...
  parts: {
    radiator: {
      ...
      mass: 5,
      specs: [
        ['Core', '620 x 400 x 42 mm, brazed aluminium'],
    ...
    pump: {
      ...
      fail: [
        ...
        'This is now a single point of failure: a dead pump stops the only loop, and the car limps on the thermal mass of 8 litres of glycol, minutes at city load, less on a climb.',
    ...
    'coolant-lines': {
      name: 'Coolant lines',
      tagline: 'Eight litres in one series loop: pack, drive unit, valve, and nothing that exists twice.',
      mass: 18,
      specs: [
      ...
      how: 'Out of the pump the loop crosses the valve, runs the passenger side above the pack lid feeding battery-5\'s single brazed cooling plate, ducks under the penthouse, and lands on the rear drive unit\'s oil-to-coolant plate, the same 11 kW exchanger at the same stub positions drivetrain.js has carried since Gen 1, with the rear inverter\'s cold plate teed off the same run. It returns along the driver side. The order is deliberate: the pack gets the coolest coolant first, the drive unit\'s oil circuit tolerates the few degrees the pack adds, and the front induction unit needs nothing because Gen 1 sealed it for life. One loop, nine hoses landing on four valve ports; Gen 1 filled the octovalve\'s eight from two loops and a chiller branch, and every hose is two clamped joints, a metre of assembly labour, and about 300 g.\n\nWhat this loop no longer does is the flagship\'s most celebrated trick. Gen 1\'s lines were the preconditioning path: flip the manifold to series, push motor and compressor heat forward, and add 25 °C to a lithium pack before a fast charger, because cold lithium plates at high current. battery-5 charges cold, so the errand is deleted along with the litres, the second pump, and the modes that ran it. Three litres less glycol also means 3 kg less mass and less thermal ballast: load spikes reach the valve sooner than they did on Gen 1, and the wide-window pack absorbs them without comment.',
      why: 'Hose count is the honest unit of thermal cost, which is why Gen 1 folded five valves into one block and why Gen 5 goes further and deletes the circuits themselves. The range ledger, stated plainly: on a cold motorway WLTP the flagship\'s full system holds a 3 to 5% edge over this one, shutters, split loops and chiller each earning their keep at speed. On the 25 km/h urban cycle this car was bought for, the gap closes to about 1%, inside tyre-pressure noise, and the invoice difference never closes.',
      fail: [
...
const RX = 2.17;                        // radiator centre x
const RY = 0.50;                        // radiator centre y
