/* Interior, Gen 5 apex. NAMING: the module id carries the -6 suffix because
   ...
   - thermal-6: HVAC case x 0.78..1.00, y 0.58..0.82, z -0.25..0.31, with two
     duct stubs whose top faces sit at y 0.86, centred (0.89, 0.84, -0.10) and
  ...
  parts: {
    ...
    displays: {
      ...
      specs: [
        ...
        ['Centre display', '15 in, 3200 x 1360, optically bonded, 1400 cd/m²'],
      ...
      fail: [
        ...
        'A colour-blind driver reads amber against green poorly, so band state is carried by segment position and motion as well as hue, and the fallback is the cluster strip in words.',
    ...
    console: {
      name: 'Centre console',
      tagline: 'Still furniture, and now honest about it: nine kilograms of moulding deleted from a part with no structural job.',
      mass: 6,
      specs: [
      ...
      fail: [
        ...
        'A foam-core tub takes a dropped-object dent that a PP moulding would shrug off; the visible surfaces carry a sacrificial skin and the repair quantum is the panel rather than a polish.',
    ...
    pedals: {
      name: 'Pedal set',
      tagline: 'Two simulator elements and three position channels, because everything the driver feels through the sole is synthesised, and a synthesiser is allowed to fail exactly once.',
      mass: 5,
      specs: [
      ...
      fail: [
        ...
        'Two elements in parallel mean the pedal survives one failure and changes character doing it: the driver gets a firmer, shorter pedal with no warning at the moment of failure, and the car says why afterwards rather than during.',
        'Disagreement between the accelerator\'s three position channels zeroes the torque request immediately; the car coasts and braking is unaffected, which is the correct behaviour and still alarming at motorway speed.',
      ],
      explode: [0.35, -0.18, 0],
    },
    'canopy-liner': {
      ...
      specs: [
        ...
        ['Liner', 'Vacuum-metallised low-e upper face, ε 0.05; treated load 35 W'],
    ...
    nvh: {
      name: 'NVH package',
      tagline: 'Four motors bolted inside the wheels put a 2.2 kHz tone into the structure, and no loudspeaker on earth can cancel it: this part absorbs where cancelling is physically impossible.',
      mass: 8,
      specs: [
      ...
      fail: [
        ...
        'Absorption is a volume game and the boot and wheelhouses are the only volumes available, so a fully loaded car with luggage packed against the rear wheelhouse liners loses part of the treatment exactly on the long journeys it was bought for.',
    ...
    restraints: {
      ...
      fail: [
        ...
        'A centre bag between the front seats is a bag that fires into the space where a passenger might be reaching across; the deployment is suppressed on any classification the mat cannot resolve, which means the far-side protection is the first thing lost to a dirty sensor.',
      ],
      explode: [0.15, 0.9, 0],
    },
  },
...
/* ── Detail pass: what changed and what it is allowed to change ────────────
   ...
   2. FOUR AIR VENTS WERE INSIDE THE DASHBOARD. The vent bodies sat at
      ...
      every one of them was 25 mm inside a solid moulding, invisible from all
      six of the driver, passenger, rear and three exterior eye points. They
      are rebuilt as real bezelled grilles standing proud of the skin: bezel
   ...
   - INTERPOLATION STAYS LINEAR. Every clearance this module publishes was
     ...
     envelope extremes but moves the surface between stations by centimetres.
...
/* The same call with all twelve edges chamfered: 44 triangles against 12,
   and it is most of the difference between a moulding and a rectangle. */
...
/* Crease along GRID LINES ONLY, never across a quad diagonal.
   ...
   line is a sampling artefact. Triangle cost: zero, same as lib.crease.
...
/* An upholstered or moulded panel. Seams are the whole point: a stitch line
...
/* A dash or door air vent: a bezel frame, four directional vanes standing in
   ...
   inside the aperture: a bezel with the moulding showing through it. Setting
...
/* ── Seam sets ─────────────────────────────────────────────────────────────
   ...
   are metres, cut along the local surface normal, and a groove can only
   remove material. */
...
/* The 40/20/40 fold is a real line on a real bench and it was not drawn.
   The centre section is 20 percent of a 1.44 m bench, so the splits sit at
...
/* Dash parting lines. Two across the car, at the screen base where the
   demist grille sits and at the bottom of the fascia, and four fore and aft:
   an end cap each side where the moulding runs out to the door trim, and the
   pair that frames the centre stack. */
const DASH_SEAMS = [
...
export function build() {
  ...
  /* the moulding's own end return, so the dash stops with an edge rather
  ...
  /* Vents. They used to sit at x 1.037..1.087, which is 25 mm INSIDE the
     ...
     the car. Rebuilt as bezelled grilles whose bezel occupies x 0.9955..
     1.0155 and whose vanes occupy x 0.9980..1.0140, at y 0.806, below the
     centre display's lower edge at 0.827. The whole assembly including the
  ...
  for (const [sz, dz] of [[-0.10, -0.20], [0.12, 0.20]]) {
    /* Collar clamped over thermal's stub top face at y 0.860: the joint is
       ...
       0.951, y 0.856 to 0.876, and a 56 mm collar centred at x 0.890 reached
       ...
       top is x 0.855 to 0.905, 50 mm of it, and a 44 mm collar centred at
  ...
  /* ── displays: 15 in centre panel raised so its lower edge clears
  ...
  /* A screen is a bezel with a display plane behind it, and the bezel is
     ...
     lib.badge extrudes a frame with a real bevelled rim in one mesh. */
  ...
  centre.add(lib.cbox(0.020, 0.165, 0.368, 0.004, M.plastic));
  ...
  centre.add(cBez);
  centre.position.set(0.998, 0.990, 0.03);
  centre.rotation.set(0, -0.16, -0.10);
  disp.add(centre);
  ...
  /* 1.44 m handover band: the lens, and the trim lip it hides under. The lip
     is the reason the band survives the low sun the centre panel loses to,
  ...
  /* THE REAR POSTS STAY PLAIN, and it is measured rather than lazy. On the
     ...
     COMMENT USED TO PUBLISH WAS BACKWARDS. It said the outboard headrest
  ...
  /* the moulded guide funnel that makes an ISOFIX bar findable without
  ...
  /* the tub, its parting line, and the plinth it stands on. A sandwich tub
     is two mouldings bonded at a flange, and that flange is the one line on
  ...
  /* the armrest is the bin lid and the highest-cycle part in the cabin, so
     it gets the section a padded lid has: a chamfered core, a centre stitch
  ...
  /* the slide rails. One rail on the centreline at y 0.6145 spanned
  ...
  for (const pz of [-0.060, 0.060]) {
    /* the pad recess, framed rather than laid on: a bevelled surround with
       ...
       extrusion and the coil plate was 1.5 mm under the moulding, which
  ...
  for (const s of [-1, 1]) {
    ...
    for (const rx of [0.220, -0.100, -0.420]) {
      ...
      lin.add(riser);
      /* the bezel where the riser pierces the shell: a hole in a moulding
  ...
  for (const s of [-1, 1]) {
    /* wheelhouse block held outboard of suspension-4's rear damper body
       ...
       A moulded absorber is stepped where the wheelhouse changes section,
       ...
       Ray-swept at this station the rear tyre's inner sidewall stands at
       ...
       |z| 0.615..0.665 was 8.60 mm inside the tyre over 96 crossing pairs
       and 14.16 mm inside the cage over 164. It now runs |z| 0.616..0.652,
       11 mm off the tyre, and its 300 mm height stops at y 0.910, 7 mm
  ...
  /* An airbag housing is a can with a door on it, and the door is defined by
     its tear seam: the H the moulding is scored along so it opens where it
