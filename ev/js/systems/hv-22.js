/* GEN 22: THE FLAP CLOSES. hv-21 with exactly one change: the charge flap
   is drawn in the driving state. It had stood 83 degrees open since the
   inlet was drawn, and swept open its leaf reaches |z| 0.6883 at y 0.833
   to 0.848, the widest thing on the whole car above the pant band: the
   frontal union has been billing an open charge door to the driving
   silhouette, 0.0045 m2 the scanline probe of 2026-08-25 measured. Closed
   it lies on the shroud at about 0.579 and the band's edge returns to the
   skin. Nothing electrical moves, nothing else moves at all; the hinge
   still stands its stated 5 mm proud and the failure list still says so.
   design/gen22.md is the brief.

/* GEN 21: THE CONVERTER AFT. hv-18 with the converter moved 1.03 aft and
   60 mm down into the tail cavity (its Gen 11 address is the center
   wheel's volume now; the first draft's -2.06 stood inside the rear
   casting's crossmember, caught by box test)
   and every affected run re-derived: the pack feed around the +z side of
   the house, one center run to drivetrain-21's translated loop bracket in
   place of the two corner runs, the spine's rear links and the port run
   following the new address. The carried text below reads at the old
   address wherever it names x -1.42 or the corner runs. design/gen21.md
   is the brief.

/* HV-18: hv-15 with the charge port moved forward, because the flank it stood
   on is gone. body-18 brings the roof down from the single head, and at the
   port's datum station (x -1.845) the belt that held y 0.90 for the port on
   body-15 to 17 is at 0.73 and the skin above it slopes in to |z| 0.20
   within 40 mm; a 110 mm inlet whose rim spans y 0.665 to 0.853 stands
   through it. design/gen18.md 4b found the alternative (holding the belt for
   the port) separates the stream at 0.1285, worse than Gen 17: the port is a
   tenant and moves. It moves to x -0.850 on the rear quarter, which on body-18
   is the hatch's side panel, where the flank is vertical at |z| 0.560 from
   y 0.62 to 0.867: the same rim on the same 0.560 flank, 14 mm under the belt.
   Its pigtail runs a meter further forward along the sill spine; 0.6 kg. The
   spines, the converter, the buffer, the drops, the stubs and the 820 W lump
   carry to the vertex. Everything below is hv-15's own record. */
/* HV electrical, Gen 15: hv-11 with the charge port 40 mm inboard and the
   front zone drops with it, because body-15's flank came in 40 mm.

   TWO PLACEMENTS MOVE AND EVERYTHING ELSE IS hv-11's TO THE VERTEX. hv-11
   re-cut the charge aperture against body-11's flank at |z| 0.6000 (the rim
   on 0.5720, the flange 0.5540 to 0.5600, the door's hinge axis at 0.6110,
   5 mm proud of the skin) and landed its front zone drops beside
   interior-11's dashboard side panel, the conduit at |z| 0.548 to 0.560
   inside a skin at 0.600. body-15 draws the flank at 0.560 from the shoulder
   up. So:
     the PORT datum's z goes 0.5720 to 0.5320 and everything keyed to it
       follows (the mount, the shroud, the seal, the contacts, the lock, the
       AC pigtail), the door's hinge axis 0.6110 to 0.5710 (its base still
       5 mm proud of the skin, which on body-15 measures 0.5600 over the
       whole inlet footprint x -1.900 to -1.790 by y 0.735 to 0.845 because
       the narrowed flank now holds 0.5600 to station 15b at x -1.905 and the
       tail cone's taper begins behind it), and the V2G electronics box and
       its gland 40 mm inboard with them, to |z| 0.325 to 0.455, 5 mm inside
       the luggage liner's 0.330 at the box's inboard face: measured by the
       checker on the gen15 set rather than assumed, and the box's inboard
       face is what moves if it binds;
     the FRONT ZONE DROPS' conduit keeps its start on the corner box wall at
       |z| 0.5600 and comes 40 mm inboard over its rise, to 0.5080 at the
       top, and its connector, whose body reaches 58 mm outboard of the
       point it is aimed from, is aimed from 0.4930 so that it ends 9 mm
       inside the new skin as hv-11's ended inside the old (a first draft at
       0.5080 ended 6 mm past the skin and the checker said so); it mates
       7 mm off a dashboard side panel interior-15 moves inboard with it,
       to |z| 0.462 to 0.486.
   The rear drops run under the shoulder, which did not move, and carry.
   The spines, the converter, the buffer, the hv-runs, the by-wire feeds
   and the 820 W lump carry, and the one honest refusal to book a watt
   carries with them.

   hv-11's header follows as the record of the ring becoming two spines. */

/* (hv-11) Gen 11 HV electrical: the ring loses its corridor and becomes two spines.
...
export const SYSTEM = {
  id: 'hv-22',
  name: 'HV · Gen 22 the flap closes',
  color: 0xff8a3c,
  explode: [-1.5, 0.3, 0],
  closures: {
    'v2g-flap': {
      ...
      mass: 0.2,
      seconds: 0.4,
      seat: 'Hinged at the top on the knuckles at (-0.850, 0.842, 0.5710), the axis 5 mm proud of a flank at 0.560, closing over the shroud rim at 0.572',
      motion: [
        { kind: 'swing', axis: [1, 0, 0], pivot: [-0.8500, 0.8420, 0.5710], deg: -83 },
      ],
    },
  },
  blurb: 'hv-21 with exactly one change: the charge flap is drawn closed, the driving state. It has stood 83 degrees open since the inlet was drawn, and swept open its leaf reaches |z| 0.6883 at y 0.833 to 0.848, which the 2026-08-25 scanline probe measured as the widest thing on the car above the pant band: the frontal union has been billing an open charge door to the driving silhouette, about 0.0045 m2. Closed, the leaf lies over the shroud at 0.579 and the band edge returns to the skin. The flap is a registered closure now, swept through its 83 degrees like every other panel that opens, and the parked-versus-driving distinction the tour teaches gets its smallest and clearest example. Nothing electrical moves; the hinge still stands its stated 5 mm proud and the failure list still says so.\n\n' + 'hv-18 with the converter moved 1.03 aft and 60 mm down into the boat tail\'s cavity, because its home since Gen 11, x -1.42 on the centerline, is the center wheel\'s own volume now: the sweep that opened the rung put 1,518 converter vertices and 1,072 run vertices inside the wheel cylinder, and a wheel does not negotiate. The tail cavity gen19 named as unlined gets its first tenant. Every run that touched the old address follows: the pack feed rides the +z corridor beside the house skirt, the drive feed and thermal spur start 0.64 further aft and rejoin their sills, the port run climbs from the new address, the spine\'s two rear links extend along both corridors, and the two corner runs become one center run that rounds the wheel outboard at z -0.28 and lands on drivetrain-21\'s translated loop bracket at (-1.3375, 0.4325, -0.119). The ledger moves up, 64.5 kg against 61.6, and says so plainly: about 3.4 m of added conduit and cable is the price of the wheel standing where the converter stood, and the scenario\'s guess of -3.5 for this slot read the hv-runs ledger as if it were all corner runs, which the drawing corrected. Everything else, the port, the buffer, the zonal boxes, the stubs and the drops, carries to the vertex.\n\n' + 'hv-15 with the charge port moved from x -1.845 to -0.850, onto the rear quarter, because body-18\'s roof comes down from the single head and the flank at the old datum is no longer tall enough for a 110 mm inlet; the rim sits on the same |z| 0.560 flank at the same height, 14 mm under a belt at 0.867, the pigtail a meter longer along the sill spine, 0.6 kg. Everything else carries to the vertex.\n\n' + 'hv-11 with the charge port moved 40 mm inboard and the front zone drops with it, because body-15 brought the flank they were cut against in by 40 mm; the rim on |z| 0.532 instead of 0.572, the door\'s hinge axis at 0.571 and still 5 mm proud of a skin that measures 0.560 under the whole inlet, the electronics box with it, the drops\' conduit inside the new skin and their connectors ending 9 mm inside it, mating 7 mm off a dashboard side panel interior-15 moves with them. Everything else, the spines, the converter, the buffer, the runs and the 820 W lump, carries to the vertex.\n\n' + 'The 48 V zonal architecture is carried whole from Gen 4 and its geometry is not: a 1.20 m cabin over an unmoved pack leaves no perimeter to run a ring around and no lid to stand it on, so the loop becomes two sill spines closed through the converter and the buffer. The four by-wire stubs move off a plane that has been inside the wheels since Gen 9, onto one that is measured, and they are declared this time. 61 kg against hv-4 at 64.',
  ...
  interfaces: {
    ...
    /* GEN 21: the extent clause retires. It asserted nothing in hv-runs
       below the lid plane, which was the checkable form of "this module
       does not enter the pack" while every run lived over the pack; the
       tail legs now dive to y 0.265 under the casting's crossmember at
       x -1.82 to -2.1, a meter behind the pack's aft face, and the global
       claim stopped being the contract's meaning. Agreement and
       realization stand: the service feed's flange still seats ON the
       plane, and the battery check still sweeps the pack's own envelope. */
    'pack-lid-top': {
      kind: 'face', axis: 'y', at: 0.3080, tol: 0.002,
      part: 'hv-runs', mirrored: false,
  ...
  parts: {
    converter: {
      ...
      mass: 8,   /* carried; what moved is the address, 0.64 aft, and the runs paid for it */
      specs: [
    ...
    spine: {
      ...
      mass: 9.4,   /* Gen 21: 8 plus two rear links 1.03 longer in 70 mm2 cable */
      count: 2,
      specs: [
    ...
    'v2g-port': {
      ...
      mass: 13.6,   /* Gen 18: 13 plus 0.6 of pigtail, a meter longer along the sill spine */
      specs: [
        ...
        ['Aperture', 'Datum x -0.850, y 0.790 (Gen 18; hv-15: x -1.845); flank |z| 0.5600 measured on body-18'],
        ['Recess', 'Rim 0.5320, pins 4.5 mm behind it, flange face 0.5200 (hv-11: 0.5720 and 0.5600)'],
        ['Door', 'Hinge axis |z| 0.5710, base inner face 5.0 mm proud of the 0.5600 flank'],
    ...
    'hv-runs': {
      ...
      mass: 8.5,   /* Gen 21: 7 plus about 1.5 of rerouting around the wheel and down the tail the converter crossed */
      specs: [
    ...
    'zone-drops': {
      ...
      specs: [
        ...
        ['Landings', 'Dash side panel |z| 0.4860 (interior-15), V2G box, casting inner face 0.3450'],
...
const CX    = -2.4500;  /* GEN 21: the converter moves 1.03 aft into the tail
   cavity and 60 mm down (CDY), because its Gen 11 home at -1.42 is the
   center wheel's own volume now (swept: 1,518 converter vertices inside
   the wheel cylinder). The first relocation drafted, -2.06, stood inside
   the rear casting's center crossmember, found by a BOX test after the
   compute-deck lesson that vertex probes cannot see face-through-face;
   -2.45 sits aft of the casting's bulkhead at -2.23 with 80 mm to it,
   40-plus over the diffuser ramp, 240 ahead of the tail closure's start.
   gen19 named the cavity unlined; the converter is its first tenant.
   Old note follows. hv-4 uses -1.3200, which puts the
...
const PORT  = [-0.8500, 0.7900, 0.5320];   /* Gen 18: a meter forward onto the rear quarter, where body-18's flank is still vertical at 0.5600 from y 0.62 to 0.867 */   /* Gen 15: 40 mm inboard, body-15's flank at 0.5600 */  /* charge aperture datum, which is
   ...
   P is NOT touched. */
const VBOX  = [-0.8950, 0.7100, 0.3900];   /* Gen 18: forward with the port, 45 mm behind its datum as before; at x -0.895 body-18's flank is vertical at 0.560 to y 0.867 and the box's fins top out at 0.769 */   /* Gen 15: 40 mm inboard with the port */  /* V2G electronics, case
...
export function build() {
  ...
  const CDY = -0.035;   /* Gen 21: down to the measured headroom: the ramp under the footprint tops at y 0.3125 and the base's underside rides 12 mm over it */
  const conv = lib.part('converter', [-0.55, 0.30, 0]);
  const cvBase = lib.cbox(0.240, 0.089, 0.440, 0.007, M.castAlu);
  cvBase.position.set(CX, 0.4045 + CDY, 0);            // y 0.360 to 0.449
  conv.add(cvBase);
  const cvJoint = lib.cbox(0.256, 0.012, 0.456, 0.003, M.castAlu);
  cvJoint.position.set(CX, 0.455 + CDY, 0);            // y 0.449 to 0.461, proud 8 mm
  conv.add(cvJoint);
  const cvLid = lib.cbox(0.222, 0.029, 0.422, 0.005, M.alu);
  cvLid.position.set(CX, 0.4755 + CDY, 0);             // y 0.461 to 0.490
  conv.add(cvLid);
  const cvFins = lib.fins(0.20, 0.018, 0.38, 15, 0.003, M.alu);
  cvFins.position.set(CX, 0.499 + CDY, 0);             // y 0.490 to 0.508
  conv.add(cvFins);
  for (const bz of [-0.220, 0.220]) {
    for (const bx of [-0.090, 0.090]) conv.add(head(0.0045, M.steel, 'hex', [CX + bx, 0.461 + CDY, bz], 'py'));
  }
  for (const bx of [-0.120, 0.120]) {
    for (const bz of [-0.110, 0.110]) conv.add(head(0.0045, M.steel, 'hex', [CX + bx, 0.461 + CDY, bz], 'py'));
  }
  for (const ex of [-0.080, 0.080]) {
    for (const ez of [-1, 1]) {
      const ear = lib.cbox(0.052, 0.014, 0.044, 0.004, M.castAlu);
      ear.position.set(CX + ex, 0.371 + CDY, ez * 0.240);      // |z| 0.218 to 0.262
      conv.add(ear);
      const web = lib.cbox(0.018, 0.026, 0.018, 0.003, M.castAlu);
      web.position.set(CX + ex, 0.391 + CDY, ez * 0.227);
      conv.add(web);
      conv.add(head(0.0055, M.steel, 'hex', [CX + ex, 0.378 + CDY, ez * 0.250], 'py'));
    }
  }
  ...
  hvPlate.position.set(CX + 0.125, 0.396 + CDY, HVIN);
  conv.add(hvPlate);
  const hvShell = lib.cbox(0.024, 0.044, 0.044, 0.004, M.hv);
  hvShell.position.set(CX + 0.142, 0.396 + CDY, HVIN);
  conv.add(hvShell);
  const hvCol = lib.cyl(0.0135, 0.010, M.hv, 14);
  hvCol.rotation.z = Math.PI / 2;
  hvCol.position.set(CX + 0.159, 0.396 + CDY, HVIN);
  conv.add(hvCol);
  const hvLever = lib.cbox(0.028, 0.006, 0.016, 0.0015, M.plastic);
  hvLever.position.set(CX + 0.142, 0.4215 + CDY, HVIN);
  conv.add(hvLever);
  for (const pz of [-0.027, 0.027]) conv.add(head(0.0030, M.steel, 'hex', [CX + 0.130, 0.396 + CDY, HVIN + pz], 'px'));
  ...
  for (const [oy, oz] of [[0.440, 0.060], [0.440, -0.060], [0.392, 0.075]]) {   /* Gen 21: the port outlet rides 20 mm higher so its run's gland stays above the declared pack-lid-top, which the extent asserts for the whole hv-runs part wherever it stands */
    const boss = lib.cbox(0.012, 0.030, 0.030, 0.002, M.hv);
    boss.position.set(CX + 0.126, oy + CDY, oz);
    conv.add(boss);
  }
  ...
  for (const zs of [-1, 1]) {
    ...
    /* Gen 21: the links keep their sill exits and follow the converter
       0.64 aft, riding the corridors outside the wheel: 20 mm under the
       center run on the -z side, 30 mm inboard of the drive feed on +z. */
    const rearLink = [
      [SPX0, SPY, zs * SPZ],
      [-1.120, 0.4520, zs * 0.6100],
      [-1.190, 0.4480, zs * 0.5500],
      [-1.270, 0.4400, zs * 0.4400],
      [-1.340, 0.4250, zs * 0.3400],
      [-1.620, zs < 0 ? 0.4400 : 0.4230, zs * (zs < 0 ? 0.3180 : 0.3100)],   /* the -z link rides over the coordinator's aft ear, inside the casting's side box at 0.335 */
      [-1.740, 0.3500, zs * 0.3050],
      [-1.850, 0.2850, zs * 0.2950],
      [-1.980, 0.2650, zs * 0.2800],
      [-2.150, 0.2800, zs * 0.2600],
      [-2.310, 0.3250, zs * 0.2500],
      [-2.348, 0.3560, zs * 0.2450],
      [-2.368, 0.3460, zs * 0.2410],
    ];
  ...
  const HINGEZ = 0.5710, HINGEY = cpy + 0.0520;   /* Gen 15: 5 mm proud of a flank at 0.5600 */
  ...
  v2g.add(hingePin);
  /* Gen 22: the leaf is its OWN lib.part group wrapped in lib.hinge, the
     way body's hood and decklid are, because tools/closures.js pairs a
     declared closure with the lib.part group that carries the hinge tag and
     an inner Group is invisible to it. The part id repeats on purpose: the
     inlet's fixed hardware and its moving leaf are one part in two groups. */
  const flapPart = lib.hinge(lib.part('v2g-port', [-0.55, 0.45, 0.45]), 'v2g-flap');
  ...
  flapPivot.rotation.x = 0;    /* Gen 22: CLOSED, the driving state. hv-21
     drew it swung 83 degrees open (rotation -1.45) and the open leaf owned
     the union's edge at |z| 0.6883; closed, the leaf lies over the shroud
     at about 0.579 and the silhouette is the skin's again. The flap still
     opens in the app: the closure sweep drives this pivot. */
  flapPart.add(flapPivot);
  sys.add(flapPart);
  ...
  /* Gen 21: the feed still leaves battery-11's hatch, and its destination
     moved 0.64 aft, so it runs the +z corridor beside the wheel: z 0.27 to
     0.30 the whole way past the house skirt (|z| 0.1065 max), the spring
     (0.063) and the damper (0.118), then turns in to the wall. */
  /* the tail leg dives UNDER the casting's center crossmember (its box
     bottoms at y 0.30 over x -1.82..-2.05; the first route ran through it
     at z 0.26 and the checker said so), through the corridor between the
     box and the diffuser ramp */
  const packFeed = [
    [-1.130, 0.3220, 0.3800],
    [-1.190, 0.3550, 0.3350],
    [-1.300, 0.3900, 0.3000],
  ...
  runs.add(pfWarn);
  const pfIn = crossing(packFeed, 'x', CX + 0.1300, -1);
  ...
  runs.add(aim(gland(0.014, M.castAlu), [-2.3220, 0.3800, 0.0640], [0.24, 0.04, 0.97]));
  ...
  runs.add(aim(gland(0.008, M.castAlu), [-2.3220, 0.3800, -0.0640], [0.24, 0.04, -0.97]));
  ...
  /* GEN 21: one center run where the two corner runs were. drivetrain-21's
     loop landing is the old +z bracket translated 0.72 inboard, at
     (-1.3375, 0.4325, -0.1190); the run leaves the relocated converter,
     goes outboard to z -0.28 to round the wheel and the fork, and comes
     back in FORWARD of the fork plates (their band starts at x -1.395).
     Sampled against the built partners: 26 mm to the tire face at the
     crossing, 17 mm over the swingarm's top flange, 15 mm to the fork
     plate's forward edge, and it shares the -z corridor with the spine's
     rear link 20 mm below it and the thermal spur 50 mm outboard. */
  {
    const center = [
      [-2.3350, 0.3400, -0.1200],
      [-2.2800, 0.3500, -0.2300],
      [-2.1000, 0.2800, -0.2500],
      [-1.9500, 0.2650, -0.2650],
      [-1.8200, 0.2850, -0.2750],
      [-1.7000, 0.3600, -0.2800],
      [-1.6000, 0.4050, -0.2800],
      [-1.3800, 0.4280, -0.2000],
      [-1.3500, 0.4360, -0.1450],
      [-1.3390, 0.4330, -0.1210],
    ];
    runs.add(conduit(center, 0.007, M.hv, { pitch: 0.020, radial: 6 }));
    runs.add(aim(gland(0.007, M.castAlu), [-2.3370, 0.3395, -0.1170], [0.10, 0.05, -0.99]));
    runs.add(aim(connector(0.020, 0.020, 0.024, 0.007, M.hv),
      [-1.3360, 0.4325, -0.1155], [0.10, 0.13, 0.99]));
  }
  ...
  const portRun = [
    ...
    [-1.545, 0.6630, 0.3150],
    [-1.350, 0.6450, 0.3180],
    /* Gen 18: the run turns FORWARD to the box at -0.895 instead of aft to
       -1.89. The sill corridor beside the rear wheel is full (the corner
       inverter at y 0.50, |z| 0.61; the hub bearings; the park brake; the
       rear casting), so it goes up the tunnel side under the bay floor
       (y 0.70, |z| 0.30, 54 mm under autonomy's cold plate at 0.764 and 75
       under the floor), forward past the bay bulkhead, then down to the gap
       between hv's own runs at 0.445 and the air supply at 0.33 (y 0.385,
       |z| 0.60, under thermal-17's recovery core at 0.49 to 0.57), and up the flank to the box's inboard face from inboard, the same
       last-three-point shape the gland is found by. */
    /* Gen 21 again: autonomy-21's +z hull owns y 0.6745 to 0.764 over
       |z| 0.155 to 0.400 here, so the run passes UNDER it at y 0.63 and
       climbs only once it is forward of the hull's x -1.106 face */
    [-1.200, 0.6300, 0.3200],
    [-1.090, 0.6300, 0.3800],
    [-1.060, 0.5500, 0.4700],
    [-1.050, 0.3850, 0.6000],
  ...
  runs.add(aim(gland(0.010, M.castAlu), [-2.3260, 0.3320, 0.0780], [-0.42, 0.87, 0.26]));
  ...
  for (const zs of [-1, 1]) {
    ...
    /* Gen 15: the start stays on the corner box wall at 0.5600; the rise
       comes 25 mm inboard so the conduit keeps 32 mm inside a skin at
       0.5600, and the connector mates 8 mm off interior-15's panel */
    const front = [
      [1.086, 0.4560, zs * 0.5600],
      [1.082, 0.5200, zs * 0.5430],
      [1.076, 0.5900, zs * 0.5250],
      [1.071, 0.6500, zs * 0.5150],
      [1.068, 0.6850, zs * 0.5080],
    ];
    drops.add(conduit(front, 0.005, M.plasticLt, { pitch: 0.018, amp: 0.13, radial: 6 }));
    /* the connector's body reaches 58 mm outboard of its aim point; a first
       draft at 0.5080 put its far end 6 mm past a skin at 0.5600 and the
       checker said so; at 0.4930 it ends 9 mm inside, as hv-11's did inside
       the 0.6000 flank */
    drops.add(aim(connector(0.024, 0.020, 0.028, 0.005, M.plasticLt),
      [1.0680, 0.6900, zs * 0.4930], [0, 0.14, -zs * 0.99]));
    drops.add(aim(wrap(0.005, 0.012, M.plastic), [1.076, 0.5900, zs * 0.5250], [-0.08, 0.99, -zs * 0.04]));
    sys.add(drops);
  }
  ...
  const rearP = [
    /* Gen 18: the rear drop leaves the corner box forward to the V2G box at
       -0.895, the same dive inboard and the same climb, mirrored in x */
    [-1.224, 0.4560, 0.5600],
    [-1.200, 0.4780, 0.4900],
    [-1.170, 0.5000, 0.4300],
    [-1.120, 0.5300, 0.3900],
