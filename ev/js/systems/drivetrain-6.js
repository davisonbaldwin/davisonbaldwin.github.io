  ...
  parts: {
    'stator-rings': {
      ...
      fail: [
        ...
        'The zero-speed derate survives, softened: one coil holding the car on a curb ramp still triples its duty, and the observer that protects it still arrives exactly when a driver with a trailer wants it least.',
      ],
      explode: [0, 0, 0.97],
    },
    'rotor-rings': {
      ...
      fail: [
        ...
        'A dented can that still seals can still touch the stator across 1.0 mm; any strike that marks a can books the same runout-and-gap inspection a curbed rim already triggers on the wheels-3 side.',
      ],
      explode: [0, 0, 0.97],
    },
    'hub-bearings': {
    ...
    'service-loops': {
      ...
      fail: [
        'The loops share one routing plane and one bracket, so the redundancy is against fatigue, not geometry: the curb strike that crushes one crushes both, and the corner isolation that follows is the same event Gen 3 owned.',
        ...
        'Twice the loops is twice the fretting population and twice the cold-morning stiff cycles; the fatigue model still books sub-zero kilometers at triple weight, now across two cables per corner.',
      ],
      explode: [0, 0, 0.85],
    },
    'tv-controller': {
...
const GZ6 = -0.16;              // gear case center plane, z
...
/* Vectoring coordinator: one box in the rear drive zone, moved aft of the
   ...
   0.334, -0.28; the last centimeters are implied, house pattern). */
...
function frontUnitPart() {
  ...
  part.add(rimBolts);
  /* printed load-path ribs: seven, following the optimizer's spider */
  for (let i = 0; i < 7; i++) {
    const a = 0.35 + i * 0.82;
