/* Wheels and brakes, Gen 10: the tire, and the count it does not book.
   ...
   tire shadows stack into one, and the Crr does not move). This slot skipped
   ...
   4.5 mm and 18.6 percent, the center groove deleted for a continuous center
   ...
   4.5 bar. The diameter does not move by so much as a micron: the built tire
   ...
   on the tires panel and exported as CRR_BASIS below. The short version:
     ...
     5.5 kg/t class A tire and they give 4.23, which is a defensible
     best-in-world number. One set of multipliers, two bases, and only one of
     the two answers is a tire anybody has built.
   ...
   is nearer 1,763.8 and 36.2 short. The tires panel carries the correction
   and the covers panel carries the measurement.

   WHAT THE GEOMETRY DOES BUY, all of it measured off built mesh:
   - THE FRONT CORNER. The tire's inner sidewall moves from |z| 0.6625 to
   ...
   - AREA. The module's own built shadow falls 0.307129 to 0.289800 m2, both
     ...
     tire column on wheels-9's own convention falls 0.0868 to 0.0756. The AREA
   ...
   The front and rear centers live in FRONTZ and REARZ below and nowhere else,
...
export const SYSTEM = {
  ...
  parts: {
    ...
    'master-unit': {
      ...
      how: 'Seven generations and this one did not move A point. The hardware is carried whole from wheels-2 through wheels-3, wheels-6, wheels-7 and wheels-9: a dry pedal unit with a spring and elastomer simulator, dual ECUs on separate supplies, a ball-screw plunger block feeding two front calipers, and no push-through rod. Both hydraulic runs still terminate at (1.286, 0.484, plus and minus 0.620) and both aft harnesses at (-1.494, 0.287, plus and minus 0.616), because the calipers and the park brakes did not move either. wheels-9 could say an axle moved 140 mm and cost this unit two tube endpoints. Gen 10 changed the tire on the end of the system and cost it nothing at all, which is the strongest thing this box has ever demonstrated about designing around an interface.\n\nWhat did change is underneath it, and three calibrations are now stale. The tire\'s peak grip goes 0.740 to 0.714, its vertical rate goes 280.9 kN/m to 396.1, and its contact patch goes 113.4 cm2 to 89.0. Every one of those is an input to something this unit schedules.\n\nThe anti-lock threshold is set against a slip curve whose peak has moved down 3.5 percent and, more importantly, whose shape has changed: a 21 percent smaller patch on a stiffer casing reaches peak slip at a lower slip ratio and falls off it faster, so a threshold tuned for wheels-9 releases late. The pad conditioning schedule, a few newtons of pad drag metered at about 0.17 Wh per event when autonomy-4\'s forward model predicts a hard stop, is set against a deceleration this tire no longer delivers, so it now over-conditions. And the vectoring coordinator arbitrates yaw moment against a front-to-rear brake split whose lateral capacity fell at both ends by the same 3.5 percent, which is the one of the three that does not need retuning, because a uniform scaling of both axles leaves the balance where it was.\n\nThe fallback is unchanged and this time the reason is A measurement rather than an assertion. About 0.33 g on wheel motors and two park-brake drums with hydraulics gone. wheels-7 published 0.38 and then 0.33 after the tire gave up 13 percent of its peak, so the obvious worry is that another 3.5 percent takes it lower again. It does not, because 0.33 g is limited by wheel-motor torque and not by grip: the demand is a coefficient of 0.33 and the tire still has 0.714, so there is more than a factor of two of grip behind the number. That is worth printing rather than assuming, because for two generations this box has been where a regressed safety figure gets printed and this is the first generation where the tire got worse and the figure genuinely did not.',
      why: 'A control box that outlives seven generations of the hardware it commands is the strongest argument any part on this car makes, and it survives because it was designed around an interface rather than around a vehicle. What this generation adds is the opposite demonstration to Gen 9\'s. Gen 9 moved a whole axle 140 mm and this unit needed two coordinates changed. Gen 10 moved nothing this unit can see and yet invalidated three of its calibrations, because they are tuned against the tire and not against the geometry.\n\nThat is the more dangerous of the two cases and it is the one nothing checks. tools/check-interfaces.sh sweeps vertices; there is no tool on this project that notices when a partner\'s spec row moves under a calibration. Writing it down on the panel is the whole of what is available.',
      fail: [
        'The arbitration line is still the single point it was in Gen 3, a disagreement between braking and vectoring settled by a 2 millisecond priority rule that has to be right every time. This generation adds a stale input rather than a new risk: the slip curve the rule arbitrates against belongs to a tire with 21 percent less contact patch and a 41 percent stiffer casing, and nobody has re-measured it.',
        'The 0.33 g fallback was computed in Gen 7 against a drivetrain that did not exist yet, and drivetrain-9 now does. That number has still not been re-derived from the built machine, and this module did not do it either, because doing so properly means modeling short-duration rim-side inverter derate at low speed rather than reading a spec row. It is carried as an estimate with its inputs named, and at seven generations it is the oldest open item in this file.',
