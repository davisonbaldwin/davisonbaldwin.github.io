/* Wheels and brakes, Gen 7: the module that unpins the aero.
   ...
   rejected body-7's Cd 0.105 in favor of 0.118, and the cause was not the
   ...
   haunch is pinned at 0.950 half-width because wheels-6's 185-section tire
   ...
   - rear wheel centers move to (-1.45, 0.355, +-0.74); front unchanged at
     (+-1.45, 0.355, +-0.81);
   ...
   built meshes against the relocated wheel center:
   ...
   - explode matches exactly: drivetrain-7 rings +-0.900, its corner groups
     +-0.780; wheels-7 rear wheel, tire and park brake +-0.780, rear cover
   ...
   center, z along the axle, s = sign(world z) so +s is outboard. The rear
...
export const SYSTEM = {
  ...
  color: 0xd0d6db,
  explode: [0, 0, 0],
  blurb: 'The corner that lets the body finish its tail. Rear wheel centers move 70 mm inboard to |z| 0.74, which releases the haunch pin that cost body-7 thirteen counts of Cd, and the tire goes narrow, tall and hard for Crr 3.8 kg/t against wheels-6\'s 4.6. 78 kg, and about 91 miles collected. The honest half: the lateral limit falls from 0.85 g to 0.74, a 100 to 0 stop grows five meters, and the 69 miles of aero this module unpins are not collected in Gen 7, because body-7 carries unchanged.',
  ...
  interfaces: {
    'front-halfshaft-outboard': {
      ...
      note: 'hub face inboard plane, ET47 off a rim centerline at |z| 0.81. The titanium insert ring spans |z| 0.7625 to 0.7825.',
    },
  },
  parts: {
    tires: {
      ...
      specs: [
        ...
        ['Mass per tire', '7.0 kg (wheels-6: 8.0), tread depth 5.5 mm new'],
        ['Maturity', 'Production practice; the 4.4 bar casing rating is pilot line'],
      ],
      how: 'The outside diameter was decided before this module opened its file. The wheel center sits at y 0.355 in design/gen4.md\'s hard-point table, four other modules are dimensioned off it, and a tire whose crown does not reach the ground is a tire that floats, so the radius is 0.355 and the OD is 0.710. That freeze is worth arguing with, because a larger tire genuinely rolls better, so the arithmetic was run: 10 mm more radius raises the whole car 10 mm, which at body-7\'s 1.85 m of width adds about 0.0185 m2 of frontal area, and at the audited Cd 0.118 that is 0.0022 m2 of CdA on a 0.247 base. The rolling gain from 2.8 percent more radius is worth roughly the same. It is a wash that moves a hard point four modules stand on, so the OD stays and everything else changes. 155/65 R20 lands the frozen diameter at 0.7095 m, half a millimeter under, which is a fifth of the tire\'s own build tolerance. Section width falls 185 to 155, sidewall height rises 92.5 to 100.75 mm, and the rim comes down from 21 inches to 20, reversing a wheels-2 decision on purpose: that module traded sidewall for rolling loss at 185 mm of section and 3.4 bar, where carcass deflection was the problem. At 155 mm and 4.4 bar the deflection is already small, the marginal Crr of 8 mm more sidewall is under half a percent, and the ride debt three generations logged is real.\n\nThe Crr ledger is built from four multipliers and it deliberately underclaims. Tread band 16 percent narrower, and tread is roughly 60 percent of a radial\'s hysteresis, gives 0.903. Pressure 3.4 to 4.4 bar, with Crr scaling near p to the minus 0.4, gives 0.93. Tread depth 7.5 to 5.5 mm on a low-void pattern gives 0.96. A next-generation functionalized-polymer silica compound gives 0.95. The taller sidewall gives 1.004 back. The stack multiplies to 0.769, and wheels-6\'s 4.6 is an installed figure rather than a label, so the modeled result is an installed 3.54. The number carried into js/efficiency.js is 3.8, which is 7 percent worse than the model on purpose. This ladder has measured its own label-to-road gap once already, wheels.js published 6.5 and reported 7.2 installed, and 11 percent applied to 3.54 would land at 3.9. Claiming 3.8 keeps two thirds of that gap as margin instead of booking the model, which is the direction to be wrong in when a whole generation is sized off one coefficient. What it buys is exact. Rolling force is Crr times mass times g, the Gen 7 preset masses 1,404 kg, so 0.0008 of Crr is 11.0 N of road load removed at every speed. That is 4.93 Wh per mile at the wheels, 5.30 out of the pack at drivetrain-6\'s measured 0.945 and battery-6\'s measured 1.016 round trip. Against the generation\'s 105 Wh/mi target on 190 kWh, range goes from 1,723 miles to 1,809: this tire alone is worth about 86 miles.',
      why: 'design/gen7.md ranks rolling resistance third among the levers and calls it the least exploited on the ladder, and the reason it stayed unexploited is that every previous generation had a second axis to protect. Gen 2 protected the lateral limit and stopped at 5.2. Gen 5 protected body-6\'s arch clearances and froze the envelope, taking 4.6 out of construction alone. Gen 7 has one metric and therefore no excuse: it spends the width, the pressure, the tread depth and the compound in the same direction and takes the grip loss on the chin, because a generation that optimizes an outcome has to be willing to be worse at something.',
      fail: [
        'The lateral limit falls from about 0.85 g to about 0.74, a 13 percent loss, because cornering stiffness tracks section width and this tire gives up 16 percent of it while running a compound chosen for low hysteresis. That is the largest single capability regression anywhere on this ladder, it is not recoverable by software, and stability control can only manage the smaller margin rather than restore it.',
        'Braking is worse and the arithmetic is public. Contact patch area is load over pressure and length is area over width, so 29 percent more pressure against 16 percent less width shortens the patch about 8 percent, and peak mu falls with the compound. At mu 1.05 a 100 km/h stop is 37.5 m; at 0.92 it is 42.7 m. Five meters is a car length and a half, and the wet case is worse still because the rear tires no longer run in the fronts\' cleared track: the rear axle is 70 mm inboard per side, so on standing water it meets water the front tire did not move.',
        'Usable tread life is roughly halved in wet-legal terms. 5.5 mm new against 7.5 leaves 3.9 mm above the 1.6 mm legal floor rather than 5.9, so the tire reaches the end of its wet performance at about two thirds of the distance. Hydroplaning onset itself improves, because the threshold rises with the square root of pressure and 4.4 bar puts it near 133 km/h against 117 at 3.4, so the loss is available grip on a merely wet road, not sudden flotation. The honest summary is that this tire is safer in deep water and worse in the rain.',
      ],
      explode: [0, 0, 0.85],
    },
    'front-wheels': {
      ...
      how: 'The construction argument is wheels-6\'s and it is not reopened: barrel, ten integrally woven spokes and hub face laid up as one autoclaved preform over a mandrel with a titanium insert co-cured into the hub face, so there is no bond line to fatigue and no forged spider to carry. What changes is the size of the thing. A 155 section on a 20 inch rim needs a 5.0J barrel where the 185 needed 6J, and one inch less diameter takes the bead seat from r 0.2665 to 0.254. Both are mass at the largest radius on the car: 0.5 kg per wheel, unsprung and rotating, which counts about one and a half times on stop-go work. The front wheel also gets the drop well the rear cannot have. There is no annulus in front of it, so the well floor sits at r 0.230, 24 mm below the bead seat, and the tire mounts on any machine. The rear wheel\'s well is 6 mm deep for a reason the next panel gives.\n\nThe front track stays at 1.62 m and that is a decision rather than an omission. The pin body-7 measured is at the haunch, over the rear tire\'s fore and aft extent from x -1.10 to -1.80; nothing forward of the rear door is held open by the front wheels, because at |z| 0.8875 the front tire already sits 37 mm inside the 0.925 beltline. Declining it is not free, and the bill is far bigger than the beltline suggests, because the beltline is not what this car shows the air down there. Integrate body-7\'s stations the way body-7 does, maximum half-width over all stations at each height: the front pods hold 1.000 to 1.007 from y 0.28 to 0.76 while the flank behind them runs 0.925, so the pods own the frontal projection across the whole lower body and the rear haunch never appears in it at all. Bring the front centers in to |z| 0.74 and the pod collapses onto that flank, worth 0.049 m2 of frontal area, 0.0058 of CdA at the audited Cd 0.118, 0.96 Wh/mi and about 17 miles. Gen 7 declines 17 miles to hold 1.62 m of front track, and it declines them on purpose: this is the axle carrying three quarters of the braking and all of the steering into a generation that has already given away 13 percent of tire grip, and narrowing both ends is how a range car becomes undriveable one defensible decision at a time.\n\nWhat the narrower tire hands back for nothing is lock, and the released amount is smaller than a single corner value suggests. body-7 sized its fairing against a 185 tire whose outer corner sits 0.0925 cos(d) plus 0.355 sin(d) from the wheel center, |z| 0.9856 at 14 degrees, and it paid a 21.4 m turning circle for it. Run the same expression on a 155: 0.0775 cos(d) plus 0.355 sin(d) does not reach 0.9856 until 16.6 degrees. Swept as a solid rather than as that one corner, against body-7\'s built loft, the fairing skin still clears by 17.5 mm at 17.6 degrees and the binding part is its arch louvers at |z| 0.9569, which touch at 17.25. Contained lock therefore goes 14.0 to 17.2 degrees, which on body-7\'s own conversion is 21.1 m curb to curb before rear steer and about 16.9 m after suspension-4\'s 4.2 m of it, against the 21.4 m body-7 published. Released, not delivered: the rack travel and the inner arch were both cut around 14 degrees and neither belongs to this module.',
      why: 'Two of this generation\'s three big moves happen at the rear axle, and the front wheel exists to prove that the third one, the tire, pays on its own. It is the same wheel wheels-6 built, one size smaller, on a track nobody moved, and it still returns half a kilogram of unsprung rotating mass and hands the next body generation a fairing that no longer has to be that wide. A range generation that only knows how to change things is a generation that will eventually change something it should not have.',
      fail: [
        'A one-piece wheel still has no repairable interface, and the smaller rim makes it worse in one specific way: a 5.0J barrel at 20 inches has less section modulus than the 6J at 21, so the curb strike that dented the Gen 5 wheel cracks this one. Carbon does not bend and confess, so the tap test and ultrasound at every tire change carry over from wheels-6 and matter more.',
        'Front and rear wheels are now further apart than they have ever been, not just in construction but in geometry: the front mounts at ET47 on a hub face 47.5 mm inboard of its rim centerline, the rear mounts on a flange 90 mm inboard of its own. They are keyed at the pilot so a rear wheel physically cannot seat on a front hub, which remains the only interlock a hurried shop respects, and the pair now also carries different tire pressures on the placard for the same size.',
        'The 3.2 degrees of lock the narrower tire released is not lock this module can deliver. It is stated as a released constraint because the swept solid is measurable against body-7\'s built loft, and the actual turning circle depends on rack travel and inner arch clearance that were both cut around a 14 degree limit. If nobody re-cuts them, the 21.4 m circle simply stands with the reason for it deleted, which is the most annoying kind of leftover.',
      ],
      explode: [0, 0, 1.15],
    },
    'rear-wheels': {
      name: 'Structural rear wheels',
      tagline: 'Seventy millimeters inboard. That is the whole of this generation\'s aero lever, and it is spent here.',
      mass: 12,
      count: 2,
      specs: [
        ['Wheel center', '(-1.45, 0.355, +-0.74); rear track 1.48 m (was 1.62)'],
    ...
    covers: {
      ...
      fail: [
        'The front brake\'s air supply is now scheduled by a louver this module does not own. body-7 opens its arch louvers on a brake-temperature model, and if that model is wrong, late, or the louver is fouled, the disc pumps whatever is in front of it and nothing announces the shortfall the way hv-4 announced a seized blade. wheels-6 could at least command its own cover; this one has a fixed geometry and a neighbor\'s schedule.',
        'A full cover is a full cover in a curb strike too. wheels-6\'s blades could be individually replaced and its bezel seated inside the rim mouth; this dish stands proud of the tire section at its outer edge, which is the first thing a curb touches, and a cracked cover is a whole cover. It is also 312 mm of unsupported radius retained only at its bezel ring, so the resonance case at highway wheel speeds is a real design item and the honest note is that it is closed on a rig rather than by a fleet.',
    ...
    calipers: {
      name: 'Front calipers',
      tagline: 'For the first time the caliper is not the limit and the tire is, and the design margin stays anyway.',
      mass: 6,
      count: 2,
      specs: [
      ...
      how: 'The casting is wheels-6\'s and the construction argument is Gen 2\'s: one forging machined into a fixed monobloc so hard-stop fluid volume becomes clamp force instead of bridge flex, six 32 mm pistons in three pairs so a long pad cannot taper, titanium piston crowns conducting at 22 W/mK rather than steel\'s 50 to keep C/SiC backplate heat out of the fluid, and rollback seals pulling the pads 0.1 mm clear so a brake used a twentieth of the time is not rubbing the rest of its life. Two things changed around it. The ring grew 6 mm in thickness, so the two halves move apart: the inboard half now sits centered at |z| 0.6535 and the outboard at 0.7245, each straddling the friction faces at 0.669 and 0.709 with a 0.5 mm pad gap, and the bridge crosses outside r 0.1825 at |z| 0.689. The pads grow in area to suit the thicker ring and the higher continuous flux, which is where the caliper\'s mass stays flat despite nothing else being added.\n\nThe more interesting change is that this caliper is now demonstrably oversized, and the module keeps it anyway. Peak deceleration is set by the tire, and the tire gave up 13 percent of its limit this generation, so the brake torque the front axle can actually use fell with it, and the gap is not marginal. Two clamps of 38 kN on a 164 mm pad mean radius at a C/SiC pad mu of 0.35 is 4.36 kNm per corner. A 0.74 g stop on 1,404 kg is 10.2 kN at the road, about three quarters of it at the front once transfer has finished, which is 1.36 kNm per front corner. The caliper delivers a little over three times what the tire will accept before ABS intervenes, and wheels-6 sized it against a 0.85 g tire on a 1,363 kg car that took 1.51. The temptation is a smaller caliper and a kilogram of unsprung mass. It is refused for the reason wheels-3 and wheels-6 both gave and this generation strengthens: these two calipers are the entire hydraulic system of the vehicle, one pressure block, two lines, twelve pistons, not a milliliter of fluid anywhere else, and the certification argument about stopping a car with every electron gone runs through this component pair alone. Peak torque is not what sizes it. Fade-free clamp after months of disuse, at the end of a descent, with a rear axle that contributes under 8 percent, is what sizes it, and none of those cases got easier.',
      why: 'The honest reading of an oversized caliper on a low-grip tire is that the margin migrated rather than disappeared. wheels-6 sized this caliper against a tire that could use all of it; Gen 7 keeps the hardware and lets the surplus sit on the side of the failure case rather than the performance case, which is what a car with no cost ceiling should do when the alternative is a kilogram. Where that argument would be dishonest is if the module then claimed the kilogram, so it does not: the mass ledger carries 6 kg here, unchanged from wheels-6, and the saving this module reports comes from tires, wheels and covers.',
      fail: [
        'Two calipers means two single points, unchanged since Gen 2, and the rear axle now offers even less behind the mistake than it did in Gen 5 because the tire behind it grips less too. The dual-circuit block and honest inspection remain the whole defense.',
...
/* ── Geometry ──
   ...
   Key world planes, rear corner:
     ...
     0.8175  tire outer face (was 0.9025 on wheels-6)
   ...
   Key world planes, front corner (unchanged from wheels-6):
     ...
     0.7325  tire inner face
     0.7625  hub face plane
     0.8875  tire outer face (was 0.9025)
     0.8890  cover outer lip */

const REARZ = 0.74;              // rear wheel center |z|, the Gen 7 move
...
/* ── Tires ──
   ...
   band, crown radius P.tireR so the tire reaches the ground rather than
...
/* ── Front wheel ──
   ...
   which puts the offset at ET47 measured from the rim centerline. The well
...
function rearWheelPart(s) {
  ...
  /* hub pilot, outboard of the annulus at |z| 0.730 to 0.756. Drawn as a
     ...
     millimeter of fit clearance. wheels-6 drew a solid 0.072 cylinder
  ...
  /* center web: ten spokes, pilot to drop well, outboard of the annulus.
     0.180 long on a 0.160 center radius so the inner ends bury into the
...
/* ── Covers ──
   ...
   so its edge stands 1.5 mm proud of the tire section maximum. The 6 mm
   gap from that edge to the tire shoulder is the brake exhaust slot.
...
function coverPart(s, rear) {
  ...
  const face = zlathe([
    ...
    [0.2850, 0.0760],
    [0.3060, 0.0785],
    [0.3120, 0.0790],   /* outer lip, 6 mm from the tire shoulder */
  ], s, M.carbon, RSEG);
  g.add(face);
...
/* ── Front discs ──
   ...
   diameter: suspension-4's carried upper ball joint (center radial 0.210,
...
/* ── Front calipers ──
   ...
   pad gap each side: halves centered at |z| 0.6535 and 0.7245, bridge
...
function caliperPart(x, z) {
  ...
  /* six pistons, three a side, crowns on the friction faces at |z| 0.669
     and 0.709, centered at radial 0.162 with +-0.038 of tangential spread
...
function masterUnitPart() {
  ...
  /* aft harnesses: park-brake commands plus the brake-priority arbitration
     ...
     radial 0.224 from the wheel center, inside the r 0.155 to 0.235 band,
