...
export const SYSTEM = {
  ...
  parts: {
    ...
    spats: {
      ...
      mass: 12.2,   /* Gen 20: the wall keeps its area (1.001 m2 against 0.991, tools
...
function buildShell(sys) {
  ...
  const SPAT = [
    ...
    [-1.030, 0.7800, 0.4600], [-1.095, 0.8055, SPAT_TOP], [-1.440, 0.8055, SPAT_TOP],
    ...
    [-1.780, 0.8055, SPAT_TOP], [-1.900, 0.8000, SPAT_TOP - 0.010, 0.7800], [-2.060, 0.7500, SPAT_TOP - 0.030, 0.6000], [-2.250, 0.6400, SPAT_TOP - 0.060, 0.5100], [-2.400, 0.5300, SPAT_TOP - 0.085, 0.4650],
  ];
  for (const side of [-1, 1]) {
    ...
    const rows = SPAT.map(([x, hz, top, hzLo]) => {
      ...
      if (hz === 0.8055 && lo > 0.81) {
        return [[x, 0.1800, side * lo], [x, 0.3550, side * (lo - 0.0020)], [x, top, side * hz]];
      }
