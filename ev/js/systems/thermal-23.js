/* GEN 23: THE CORE FOLLOWS. thermal-22 with the recovery core's outboard
   posts, tabs and one hose end brought 30 mm inboard, |z| 0.542 to 0.512
   and 0.560 to 0.530, because body-23 brings the cabin flank from 0.560 to
   0.530 and this part was drawn against the old one. Its own geometry
   comment says as much: the free volume it was placed in is bounded
   outboard by a flank that has now moved.

   Nothing thermodynamic changes. The core is the same 1.67 m2 of matrix at
   the same 72 m3/h and the same effectiveness; what moves is four
   mounting stations and a hose waypoint, which is a planned fork of
   exactly the kind Gen 15 and Gen 16 made the last two times this surface
   moved. The checker found it, at 28.8 mm through the skin and 28.7
   through the door leaf, before it could ship.

/* thermal-22: THE OVERHANG. thermal-17 with the outdoor coil block and the
   heat pump translated -200 mm as one piece with body-22's fascia, so every
   relation each had to the intake gates ahead of it and the casting's coil
   horn behind it is thermal-17's to the digit: core at x 1.9330..2.0030,
   fan plane 1.9040, brackets on the horn's translated front face. The
   glycol loop is thermal-17's except the two hoses that feed the moved
   parts and the deck bracket that lands on the casting. Nothing aft of the
   cowl moves at all.

   WHAT THE TRANSLATION FOUND, and it is the rung's paid lesson: the nose
   was packaged with 51 mm of slack behind the coil's mount, and everything
   past that belongs to suspension's air reservoir, which is why
   suspension-22 exists (design/gen22.md section 2). A generation that
   shortens a car does not get to move only the parts it named.

   The frunk pays: the pump's tail stands at x 1.589 inside the box that ran
   1.275..1.750, and the panel states the liters. design/gen22.md is the
   brief. The carried text below reads the old stations wherever it names
   them.

   ── thermal-17's own header follows, unchanged ──────────────────────
...
const CX = 1.9680;        /* coil core plane: core spans 1.9330..2.0030. Gen 22:
     the whole nose assembly translates -200 mm together, so every relation
     the coil had to the gates ahead of it and the casting horn behind it is
     gen21's to the digit; what does NOT translate is what was already aft of
     the nose, and that is the finding this rung paid for (design/gen22.md). */
...
const FX = 1.9040;        /* fan plane, translated with the core */
...
export function build() {
  ...
  /* end plates carry the tube returns, with the formed flange and the three
     ...
     body-11's loft at x 1.9680 measures a half-width of 0.5256 at y 0.38 and
  ...
  /* glycol defrost tee, seated ON the refrigerant fin block face at x 1.9330 */
  ...
  coil.add(at(cb(0.022, 0.014, 0.440, M.plasticLt, 0.0022), 1.9320, 0.186, 0));
  coil.add(at(cb(0.022, 0.014, 0.440, M.plasticLt, 0.0022), 1.9320, 0.497, 0));
  for (const pz of [-0.212, -0.070, 0.070, 0.212]) {
    coil.add(at(cb(0.022, 0.310, 0.014, M.plasticLt, 0.0022), 1.9320, 0.341, pz));
  }
  /* the two brackets landing ON body-11's casting FRONT face. Measured by
     -x rays through the built mesh: the face is at x 1.8700 at y 0.36 and
     1.8600 at y 0.30, so the bracket is seated at 1.8700 and its bolts on
     the bracket top at y 0.3660 rather than on body-9's coordinates. */
  for (const bz of [-0.055, 0.055]) {
    coil.add(at(cb(0.062, 0.012, 0.070, M.steel, 0.0018), 1.9010, 0.360, bz));
    for (const dz of [-0.022, 0.022]) {
      coil.add(at(lib.fastener(0.0042, M.steel, 'hex'), 1.9010, 0.366, bz + dz));
    }
  }
  ...
  for (const fz of [-FZ, FZ]) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      strut.position.set(1.8810, FY + Math.sin(a) * 0.062, fz + Math.cos(a) * 0.062);
      strut.rotation.x = Math.PI / 2 - a;
      coil.add(strut);
    }
    coil.add(at(xcyl(0.026, 0.007, M.plastic, 12), 1.8810, FY, fz));
    coil.add(at(xcyl(0.017, 0.013, M.darkSteel, 10), 1.8775, FY, fz));
  ...
  /* ══ glycol circuit and the frunk deck ══════════════════════════════════
     ...
     and x 1.5900 to 1.8450 with its top face at DECKT 0.6700, which is 24.0 mm
  ...
  for (const ds of [-1, 1]) {
    ...
    gly.add(at(cb(0.034, 0.0780, 0.018, M.steel, 0.0022), 1.575, 0.6190, ds * SPARZ));   /* Gen 22: BOTH pads translate with the arm, so the tray moves as a rigid unit rather than collapsing; the aft one goes to 1.575 rather than a pure -200's 1.605, which lands on suspension-22's front structure at x 1.610..1.697. Moving only the forward pad squeezed the tray onto a 30 mm span and slid it into hv-22's buffer at 35 mm, which is why this reads the way it does. */
    gly.add(at(cb(0.034, 0.0780, 0.018, M.steel, 0.0022), 1.814, 0.6190, ds * 0.505));   /* Gen 22: 2.014 translated -200 with the arm it stands on, and outboard from SPARZ 0.344 to 0.505 with it. At the translated station the pad's top 33 mm stood inside hv-22's buffer, and the only clear track on the arm is outboard of BOTH the buffer (|z| to 0.420) and the crash rail (0.395..0.485); the arm itself runs to 0.535, so the pad lands on 30 mm of rail with 11 mm to the crash rail beside it. Measured, not chosen. */
    gly.add(at(lib.fastener(0.0038, M.steel, 'hex'), 1.575, DECKT, ds * SPARZ));
    gly.add(at(lib.fastener(0.0038, M.steel, 'hex'), 1.814, DECKT, ds * 0.505));
    ...
    gly.add(at(cb(0.026, 0.2500, 0.026, M.steel, 0.0024), 1.855, 0.4950, ds * 0.275));   /* Gen 22: 288 mm becomes 250 and the strut ties to the tray's underside rail at y 0.620 instead of its deck at 0.658, because at the horn's translated station hv-22's buffer occupies everything above 0.625; the strut also goes outboard from |z| 0.135 to 0.275, the horn's own edge, for the same reason. Both are stated rather than absorbed: this is the third thing the 200 mm cut moved that the brief did not name. */   /* Gen 22: follows the horn it lands on (x 2.000..2.070 translated to 1.800..1.870) and goes outboard from |z| 0.135 to 0.275 with it, because at the horn's new station the inboard track is under hv-22's buffer and the horn's own edge at 0.280 is the last place a strut can stand */
  }
  ...
  /* Gen 22: the first four stations are on the coil and translate with it;
     the fifth is on the drive flange, which did not move. */
  gly.add(hose([
    [1.904, 0.192, 0.026],
    [1.898, 0.300, 0.028],
    [1.892, 0.410, 0.030],
  ...
  /* Gen 22: the tail feeds the coil and the coil translated 200 mm aft, so
     the last four stations follow it; the head stays on the deck, which did
     not move, and the hose takes the bend instead of the fittings. */
  gly.add(hose([
    [1.999, 0.731, -0.180],
    [2.036, 0.700, -0.164],
    [2.040, 0.630, -0.150],
  ...
  hp.add(at(cb(0.150, 0.012, 0.100, M.steel, 0.0018), 1.7300, 0.676, 0.230));
  hp.add(at(xcyl(0.050, 0.150, M.castAlu, 20), 1.7300, 0.732, 0.230));
  hp.add(at(xcyl(0.044, 0.030, M.darkSteel, 16), 1.6400, 0.732, 0.230));
  /* the shell is two deep-drawn halves welded at a girth seam, and the seam
     ...
     tube */
  for (const [sx, sr] of [[1.6980, 0.0512], [1.6600, 0.0470]]) {
  ...
  for (const [fx, fz] of [[1.6800, 0.196], [1.6800, 0.264], [1.7800, 0.196], [1.7800, 0.264]]) {
  ...
  hp.add(at(lib.cyl(0.010, 0.012, M.castAlu, 8), 1.7300, 0.786, 0.230));
  const suction = lib.cyl(0.011, 0.020, M.castAlu, 8);
  suction.rotation.x = Math.PI / 2;
  hp.add(at(suction, 1.7450, 0.732, 0.174));
  hp.add(at(aim(lib.fastener(0.0068, M.darkSteel, 'hex'), 0, -0.6217, -0.7832), 1.7880, 0.6997, 0.1893));
  /* terminal box with a lid, four screws and the gland the HV feed enters */
  hp.add(at(cb(0.055, 0.035, 0.050, M.plastic, 0.0028), 1.6700, 0.802, 0.230));
  hp.add(at(cb(0.048, 0.005, 0.043, M.plasticLt, 0.0010), 1.6700, 0.822, 0.230));
  for (const [sx, sz] of [[-0.020, -0.016], [0.020, -0.016], [-0.020, 0.016], [0.020, 0.016]]) {
    hp.add(at(lib.fastener(0.0024, M.steel, 'torx'), 1.6700 + sx, 0.8245, 0.230 + sz));
  }
  const gland = lib.cyl(0.009, 0.016, M.darkSteel, 8);
  gland.rotation.x = Math.PI / 2;
  hp.add(at(gland, 1.6650, 0.802, 0.260));

  /* receiver, with the straps, the sight glass a technician reads the charge
     on and the port a gauge goes on */
  hp.add(at(xcyl(0.030, 0.086, M.steel, 16), 1.7250, 0.706, 0.345));
  for (const cs of [-1, 1]) {
    hp.add(at(lib.sphere(0.030, M.steel, 12), 1.7250 + cs * 0.043, 0.706, 0.345));
    const band = lib.torus(0.0325, 0.0040, M.darkSteel, 14, 5);
    band.rotation.y = Math.PI / 2;
    hp.add(at(band, 1.7250 + cs * 0.026, 0.706, 0.345));
  }
  hp.add(at(aim(sightGlass(0.010), 0, 1, 0), 1.7550, 0.736, 0.345));
  hp.add(at(servicePort(0.0050, M.plasticLt), 1.6950, 0.736, 0.345));
  ...
  hp.add(at(lib.cyl(0.026, 0.090, M.steel, 14), 1.8100, 0.715, 0.320));
  hp.add(at(lib.sphere(0.026, M.steel, 12), 1.8100, 0.760, 0.320));
  hp.add(at(lib.cyl(0.010, 0.024, M.steel, 8), 1.8100, 0.658, 0.320));
  const aStrap = lib.torus(0.0285, 0.0045, M.darkSteel, 14, 5);
  aStrap.rotation.x = Math.PI / 2;
  hp.add(at(aStrap, 1.8100, 0.696, 0.320));
  hp.add(at(servicePort(0.0046, M.plasticLt), 1.8100, 0.782, 0.320));
  ...
  hp.add(at(plateHX(0.062, 0.074, 0.054, 10, M.steel, { portSide: -1 }), 1.8080, 0.720, 0.145));
  hp.add(at(plateHX(0.070, 0.080, 0.060, 11, M.steel, { portSide: -1 }), 1.7300, 0.820, 0.160));
  hp.add(at(plateHX(0.058, 0.070, 0.052, 9, M.steel,
    { portSide: 1, portL: 0.008, portInset: 0.012, ports: [[1, 1], [-1, -1]] }), 1.8100, 0.800, 0.250));
  ...
  hp.add(at(plateHX(0.066, 0.072, 0.056, 11, M.steel, { portSide: -1 }), 1.6600, 0.822, 0.330));
  for (const ws of [-1, 1]) {
    hp.add(at(cb(0.003, 0.076, 0.060, M.alu, 0.0008), 1.6600 + ws * 0.0345, 0.822, 0.330));
  }
  hp.add(at(lib.cyl(0.006, 0.034, M.plasticLt, 8), 1.6930, 0.875, 0.330));
  ...
  hp.add(at(cb(0.046, 0.044, 0.046, M.castAlu, 0.0035), 1.6120, 0.630, 0.190));   /* Gen 22: 65 mm down, because the translated pump put this box inside hv-22's spine at y 0.683..0.708 */
  hp.add(at(actuator(0.015, 0.028, M.darkSteel), 1.6120, 0.717, 0.190));
  hp.add(wire([
    [1.6220, 0.732, 0.190],
    [1.6380, 0.729, 0.202],
    [1.6500, 0.716, 0.224],
  ...
  for (const [px, py, pz] of [[1.6380, 0.630, 0.190], [1.6120, 0.630, 0.216], [1.6120, 0.652, 0.164]]) {
  ...
  /* Gen 22: 40 mm down. The translated pump put these two bosses inside
     hv-22's spine, which runs y 0.683..0.708 the length of the sill. */
  for (const ez of [0.135, 0.180]) {
    hp.add(at(cb(0.026, 0.030, 0.026, M.sensor, 0.0024), 1.7000, 0.646, ez));
    hp.add(at(cb(0.013, 0.012, 0.011, M.plastic, 0.0012), 1.6815, 0.646, ez));
  }
  /* hydrocarbon sensor at the enclosure low point and the overboard vent.
     ...
     hang. */
  hp.add(at(cb(0.030, 0.026, 0.026, M.sensor, 0.0026), 1.7500, 0.684, 0.360));
  hp.add(at(cb(0.012, 0.012, 0.013, M.plastic, 0.0012), 1.7500, 0.701, 0.360));
  hp.add(at(lib.cyl(0.013, 0.050, M.plasticLt, 8), 1.8060, 0.695, 0.360));
  hp.add(at(lib.cyl(0.017, 0.005, M.plasticLt, 10), 1.8060, 0.7225, 0.360));
  hp.add(at(lib.cone(0.016, 0.010, M.plasticLt, 10), 1.8060, 0.7300, 0.360));
  ...
  pk.add(hose([
    ...
    [2.020, 0.690, -0.320],
    [2.034, 0.650, -0.290],
    [2.038, 0.640, -0.250],   /* Gen 22: over the translated bumper beam (y 0.505..0.595) rather than through it */
    [1.985, 0.610, -0.190],
    [1.960, 0.490, -0.100],
    [1.960, 0.588, 0.030],
    [1.900, 0.592, 0.110],
  ...
  /* THE THREE LOCAL MACHINES ARE UNDER THE DECK, NOT ON IT, and that is the
     ...
     SPARS: x 1.720 to 1.9700 by y 0.400 to 0.580 across |z| 0.330 has no
  ...
  loc.add(at(cb(0.012, 0.130, 0.140, M.alu, 0.0018), 1.9590, 0.470, 0.070));
  for (const by of [-0.050, 0.050]) {
    loc.add(at(aim(lib.fastener(0.0032, M.steel, 'hex'), -1, 0, 0), 1.9530, 0.470 + by, 0.135));
  }
  /* the 3 W pump, its saddle on the bracket, and the mixing valve */
  loc.add(at(xcyl(0.016, 0.050, M.darkSteel, 14), 1.9100, 0.500, 0.075));
  loc.add(at(xcyl(0.0145, 0.008, M.castAlu, 12), 1.8810, 0.500, 0.075));
  const locSeam = lib.torus(0.0165, 0.0022, M.steel, 12, 5);
  locSeam.rotation.y = Math.PI / 2;
  loc.add(at(locSeam, 1.8960, 0.500, 0.075));
  loc.add(at(cb(0.020, 0.015, 0.018, M.plastic, 0.0014), 1.9040, 0.5165, 0.075));
  loc.add(wire([
    [1.9040, 0.524, 0.075],
    [1.8920, 0.538, 0.061],
    [1.927, 0.544, 0.043],
  ...
  loc.add(at(locSaddle, 1.9220, 0.500, 0.075));
  for (const ss of [-1, 1]) {
    loc.add(at(cb(0.006, 0.036, 0.007, M.darkSteel, 0.0009), 1.9220, 0.4735, 0.075 + ss * 0.0185));
  }
  loc.add(at(xcyl(0.023, 0.018, M.plastic, 14), 1.9440, 0.500, 0.075));
  for (let i = 0; i < 4; i++) {
    const a = (i / 4) * Math.PI * 2;
    loc.add(at(aim(lib.fastener(0.0028, M.steel, 'hex'), 1, 0, 0),
      1.9530, 0.500 + Math.sin(a) * 0.017, 0.075 + Math.cos(a) * 0.017));
  }
  /* mixing valve, its actuator plug on -x and its third pad on -z so neither
     faces the plate stack behind it */
  loc.add(at(cb(0.042, 0.042, 0.042, M.castAlu, 0.0035), 1.8750, 0.452, 0.075));
  const mixAct = actuator(0.013, 0.024, M.darkSteel);
  mixAct.rotation.y = Math.PI;
  loc.add(at(mixAct, 1.8750, 0.473, 0.075));
  loc.add(wire([
    [1.920, 0.485, 0.075],
    [1.906, 0.488, 0.059],
    [1.898, 0.482, 0.043],
  ], 0.0026, M.plasticLt));
  for (const [px, pz] of [[1.8750, 0.054], [1.8750, 0.096], [1.909, 0.075]]) {
  ...
  for (const [px, pz] of [[0.798, 0.358], [0.927, 0.358], [0.798, 0.512], [0.927, 0.512]]) {   /* Gen 15: the outer posts 40 mm in */
    rec.add(at(cb(0.016, 0.255, 0.016, M.plasticLt, 0.0022), px, 0.6125, pz));
  }
  for (const py of [0.491, 0.734]) {
    rec.add(at(cb(0.145, 0.012, 0.176, M.plasticLt, 0.0022), 0.8625, py, 0.438));   /* Gen 23: the frame rail was 200 mm deep from z 0.350 to 0.550 and body-23's flank is at 0.530, so it comes to 176 and spans 0.350 to 0.526: the inboard end still lands on the core's own face and the outboard end is inside the paint. The posts moving alone did not fix this, which the checker said twice at 28.8 and 28.7 mm before the rail was found. */
  }
