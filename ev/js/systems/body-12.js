...
export const SYSTEM = {
  id: 'body-12',
  name: 'Body · Gen 12 the shape',
  color: 0xdcf2fb,
  explode: [0, 1.7, 0],
  blurb: 'body-11 with its rear redrawn, which design/cd-rezero.md measured as the correction the ladder owed: the wide pan ended at x -1.07 in a 58.8 degree plan break and the rear tires stood in clean air carrying 22 percent of the car\'s drag, so Gen 11 shipped the best area on the ladder under the worst shape, 0.2397 against a Gen 1 sedan\'s 0.2018. Everything forward of the rear arch carries: the tandem cabin at 1.2000 m, the roof at 1.3300, the shoulder over battery-11\'s sill, the doors, the hood, the occupant gate. The rear is this generation. Spats take the loft out to |z| 0.8400 over the rear wheels, 32.5 mm outside the tire face, and their skirts run to y 0.1800 so the underbody keeps its side wall from the sill to the diffuser. The tail gets its length back, 4.85 to 5.10 m, and every station is drawn against the equivalent-cone walk: crown line 13.3 degrees, flank plan 12.9, and the measured flow rides to the Kamm on both streams with a base of 0.54 m2 against body-11\'s separated 1.57. The area cost is 0.0304 m2 and the mass cost is 23 kg, both booked. No coefficient is asserted anywhere in this module: tools/aero.sh measures the drawn triangles and whatever it returns is the number.',
  interfaces: {
  ...
  closures: {
    ...
    hood: {
      ...
      mass: 6.5,
      seconds: 1.2,
      seat: 'Aft cut at x 1.2700, the glass base, axis carried up to y 1.0760, 8 mm above the panel it hinges',
      motion: [
        { kind: 'swing', axis: [0, 0, 1], pivot: [1.2700, 1.0760, 0], deg: 60 },
      ],
    ...
    decklid: {
      ...
      mass: 6,
      seconds: 1.4,
      seat: 'Front cut at x -1.1600, axis carried up to y 1.3220, which is 9.2 mm above the panel it hinges on this body: the crown is already falling at the cut, 1.3128 at x -1.160 against body-11\'s 1.3076',
      motion: [
  ...
  parts: {
    ...
    skin: {
      ...
      specs: [
        ...
        ['Stations', '23 rings, x +2.375 to -2.6900; nose x held as crash budget, glass base at 1.250'],
    ...
    'tail-cone': {
      ...
      specs: [
        ...
        ['Kamm face', 'x -2.6900, 0.810 m wide by y 0.265 to 0.952'],
...
const STATIONS = [
  ...
  /* ── THE COWL, redrawn for Gen 12 (design/gen12-front.md). body-11's hood
     came down under the driver's eye line and its glass still had to reach
     the 1.322 crown by x 0.300, and what that arithmetic left at x 1.030 was
     a 45 degree ramp: the section march measures the hood-to-windshield
     break at 41 degrees, the steepest on the ladder beside Gen 1's, and the
     forebody term prices it at 0.0087. So the glass base moves forward to
     x 1.250 at y 1.070, 28 mm over interior-11's dashboard peak of 1.042 at
     x 1.20 and 22 mm under the eye ray at 1.092, and rises at a constant 25
     degrees through 1.130 at x 1.120 and 1.172 at 1.030 to station 7's
     1.256. The break falls to about 19 degrees. The hood's aft cut moves to
     1.270 so the lid ends where the glass begins. Nothing forward of 1.450
     moves, and every raised crown sits inside the cabin silhouette, so the
     frontal-area union does not see it. ── */
  /* 5  glass base, ARCH. 1.250 is 4 mm aft of the dashboard's end at 1.246 */
  [1.250, [...ARCH,
           [0.870, 0.600], [0.975, 0.540], [1.045, 0.370], [1.062, 0.190]], 1.070],
  /* 6  front shoulder closure, ARCH, on the glass line. x 1.120 is 22.0 mm
        inboard of the nearest front tire vertex at 1.0980 */
  [ARCH_X, [...ARCH,
           [0.891, 0.600], [1.010, 0.540], [1.105, 0.375], [1.124, 0.195]], 1.130],
  /* 7  the first station at full shoulder, on the glass line. x 1.030 is
        the forward end of battery-11's 28-screw run and of the pan flange */
  [SHOULDER_X, [...SHLD,
           [0.900, 0.600], [1.040, 0.545], [1.140, 0.380], [1.165, 0.200]], 1.172],
  ...
  /* ── THE REAR, and it is the whole reason this module exists.
     design/cd-rezero.md measured body-11's rear as the worst shape on the
     ladder: shoulder, floor-pan and sill all end at x -1.07 in a 58.8 degree
     plan break, and the rear tires stand in clean air behind it with
     0.1991 m2 of exposed frontal area, 22 percent of the car's drag. The
     upper stream separates at x -0.98 on a body whose tail runs to -2.44.

     Two changes, both measured in design/gen12.md section 4 before they were
     drawn. THE SPATS: the loft's lower band carries out to |z| 0.8400 over
     the whole rear wheel, 32.5 mm outboard of the tire face at 0.8075, so
     the wheel that was the largest single drag term on the car is inside the
     body. The spat truncates behind the tire in a raked step, because
     closing 0.84 to the tail inside the 15 degree limit needs 1.64 m that no
     car length has: the step is a 200 mm separation bubble by the walk's own
     rule, and body-8 shipped the same solution as an 83 mm fairing step.
     THE TAPER: the tail gets its length back, 4.85 to 5.10 m, and every
     station aft of the cabin is drawn against the equivalent-cone walk in
     tools/aero.js: crown at 13.3 degrees, flank plan at 12.9, nothing on the
     cone over 15 outside the spat bubble. Ahmed's 20 to 30 degree danger
     band is avoided on the backlight deliberately: the crown line never
     exceeds 14 degrees, which costs tail length and buys attachment. */
  /* 11 rear arch, ARCH as at the fronts: the spat is a separate wall, see
        buildShell. The rear occupant's head is over this station */
  [-1.120, [...ARCH,
            [1.020, 0.600], [1.200, 0.570], [1.293, 0.390], [1.319, 0.298]], 1.322],
  /* 12 mid spat */
  [-1.280, [...ARCH,
            [1.010, 0.598], [1.180, 0.560], [1.258, 0.385], [1.282, 0.295]], 1.285],
  /* 13 rear axle */
  [-1.440, [...ARCH,
            [1.000, 0.590], [1.150, 0.545], [1.222, 0.375], [1.245, 0.290]], 1.248],
  /* 14 aft spat */
  [-1.620, [...ARCH,
            [0.980, 0.578], [1.115, 0.528], [1.180, 0.365], [1.203, 0.283]], 1.206],
  /* 15 spat closure begins. The rear tire's last vertex is at x -1.8020 and
        the lower band still clears it by 22.5 mm */
  /* 15 spat closure begins, AND THE FLANK HOLDS THE PORT DATUM. hv-11's
        V2G port rim sits ON |z| 0.6000, measured against body-11's flank at
        its station (x -1.845, y 0.790), and hv-11 is carried. An earlier
        draft tapered the upper flank to 0.565 here and the sweep put the
        skin 9.5 mm through the port shroud: the port is a carried partner's
        geometry and the body moves around it, not it around the body. The
        rear tire's last vertex is at x -1.8020 and the lower band still
        clears it by 22.5 mm */
  [-1.800, [...ARCH,
            [0.900, 0.6000], [1.080, 0.512], [1.140, 0.352], [1.161, 0.275]], 1.164],
  /* 15b the port's aft shoulder: the shroud reaches x -1.896 and the datum
        holds to -1.905 before the tail is allowed its taper */
  [-1.905, [...ARCH,
            [0.880, 0.6000], [1.055, 0.502], [1.128, 0.346], [1.148, 0.270]], 1.140],
  ...
  [-1.950, [[0.1380, 0.5550], [0.3000, 0.5750], [0.4600, 0.5700], [0.5200, 0.5650],
            [0.6000, 0.5550], [0.7000, 0.5450], [1.0200, 0.5150], [1.1000, 0.4900]], 1.129],
  /* 17 */
  [-2.100, [[0.1550, 0.5250], [0.3200, 0.5450], [0.4600, 0.5380], [0.5400, 0.5300],
            [0.6200, 0.5200], [0.7500, 0.5050], [1.0000, 0.4720], [1.0600, 0.4500]], 1.094],
  /* 18 */
  [-2.250, [[0.1760, 0.4950], [0.3400, 0.5100], [0.4800, 0.5020], [0.5600, 0.4950],
            [0.6600, 0.4830], [0.7800, 0.4650], [0.9800, 0.4320], [1.0300, 0.4120]], 1.059],
  /* 19 */
  [-2.400, [[0.2010, 0.4650], [0.3600, 0.4750], [0.5000, 0.4680], [0.5800, 0.4600],
            [0.6800, 0.4470], [0.8000, 0.4250], [0.9500, 0.3960], [1.0000, 0.3780]], 1.024],
  /* 20 */
  [-2.550, [[0.2310, 0.4300], [0.3800, 0.4400], [0.5200, 0.4320], [0.6000, 0.4200],
            [0.7000, 0.4050], [0.8200, 0.3850], [0.9200, 0.3620], [0.9700, 0.3450]], 0.988],
  /* 21 Kamm face. Base 0.51 m2 by the drawn sections against body-11's
        measured 1.5699, and the walk stays attached to it */
  [KAMM, [[0.2650, 0.4000], [0.4000, 0.4050], [0.5400, 0.3950], [0.6200, 0.3850],
          [0.7000, 0.3700], [0.7800, 0.3520], [0.8800, 0.3280], [0.9400, 0.3080]], 0.952],
];
...
/* Hood x 2.220 to 1.270, ending where the cab-forward glass begins at ring
   5, aft of the nose fascia band. Decklid x -1.160 to -2.100. Side gaps on
   the deck shoulder, clear of the beltline seam. */
const HOOD = [rowOf(2.220), rowOf(1.270)];
...
/* THE SHOULDER'S OWN ROW SPAN, and it is the whole of blocker 2's fix. RB
   ...
   rear wheel. */
const RB = 5, RC = 12, RD = 18;   /* RB: the glass base, x 1.250. RC: the
...
function buildStructure(sys) {
  ...
  cageMid.add(lib.tube([[1.262, 1.048, -0.340], [1.262, 1.048, 0.340]], 0.016, M.darkSteel));
  sys.add(cageMid);
}
...
function buildShell(sys) {
  ...
  const SPAT = [
    ...
    [-1.050, 0.7800, 0.4600], [-1.130, 0.8400, 0.5200], [-1.440, 0.8400, 0.5200],
    [-1.780, 0.8400, 0.5200], [-1.900, 0.8000, 0.5000], [-2.060, 0.6600, 0.4600],
  ];
  for (const side of [-1, 1]) {
    const rows = SPAT.map(([x, hz, top]) => [
      [x, 0.1800, side * hz], [x, (0.1800 + top) / 2, side * (hz + 0.0020)], [x, top, side * hz],
    ]);
