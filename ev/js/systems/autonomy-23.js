/* GEN 23: THE SENSOR DUTY. autonomy-22 with two changes, one geometric
   and one that is the rung's whole range column.

   THE SENSORS GET THE DUTY ARGUMENT THIS MODULE ALREADY MAKES. The
   compute here is the most carefully power-argued thing on the car: three
   lanes, one at full rate, one a reduced-rate diverse checker, one in
   retention with clocks gated, because a scene that is 58 percent cruise
   does not need three full-rate stacks. That architecture is then fed by
   a sensor set that runs FLAT OUT in every scene, 119.4 W of lidar, radar,
   cameras and inertial, 100 percent of the time. The module argued duty
   for the consumer and never once for the supply, and tools/watts.sh is
   what made that visible by putting the two side by side.

   So the same split this file already publishes, 58 percent cruise, 30
   elevated, 12 dense, is applied to the sensors it feeds. At cruise the
   car needs forward range and not four flash lidars at full rate; in dense
   traffic it needs everything. Weighted 0.40, 0.75 and 1.00 the set
   averages 0.577 of its flat-out draw: 119.4 W becomes 68.9, a cut of
   50.5 W, which js/efficiency.js carries as this module's auxDelta going
   from -75 to -125.5 and the ladder prices at about 33 miles.

   WHAT WAKES IT, WHICH IS THE PART THAT MATTERS. The schedule is not a
   timer. Lane A holds a scene classifier at full rate, the same one that
   already escalates lane C, and any of its triggers takes the whole
   sensor set to full rate in the interval lane C already specifies, 60
   ms. Between triggers the set is not off: the forward lidar and the two
   forward radars hold full rate at all times, and it is the corner units
   and the surround cameras that drop to a third of their frame rate. A
   sensor asleep in a scene that needed it is the failure this buys, and
   the fail list below carries it in those words rather than in softer
   ones.

   THE CAMERAS FOLLOW THE FLANK, the same planned fork Gen 15 and Gen 16
   made when this surface last moved: body-23 brings it from |z| 0.560 to
   0.530, so at the lens height of y 0.945 the skin goes 0.523 to 0.493 and
   the pads, screws and bodies go 30 mm inboard with it.

/* GEN 22: THE MOVED BAND. autonomy-21 with the whole fascia sensor suite
   translated 200 mm aft on body-22's shortened nose: the lidar pucks to
   (2.13, 0.615, +-0.30), the thermal camera to (2.145, 0.53, 0), the
   front imaging radars to x 2.090 with the radome at 2.126, the front
   corner pockets to 1.940, every relative standoff held to the digit (the
   boot still clears the beam by 15.0 mm; the beam moved with the block).
   The ten-generation band-face contract at x 2.371 moves to 2.171 with
   body-22, both modules stating the same number. And the two roof GNSS
   antennas go conformal, the 9 mm foot-and-dome stack to a 4 mm patch in
   the same molding class, because the pair owned the union's top band at
   0.0006 m2 of silhouette nothing needed. design/gen22.md is the brief.
   Carried text below reads the old stations wherever it names them.

/* GEN 21: THE RING AHEAD OF THE WHEEL. autonomy-19 with exactly one
   change: the network ring's rear cross-car run moves from x -1.12 to
   -0.98, because the center tire's swept circle reaches x -1.0951 at the
   ring's y 0.365 and the carried crossing stood 25 mm inside it (swept
   2026-08-24, the find that made this fork; the probe box at the new
   station is empty in every module of the preset, and the swingarm's
   pivot bushes end 44 mm aft of it). The loops shorten 140 mm each side,
   below the ledger's kilogram resolution; every sensor, the compute, the
   cameras and the tip cluster carry to the vertex. design/gen21.md is the
   brief.

/* AUTONOMY-18: autonomy-17 with two instruments re-seated on body-18's roof
   and face. The aft GNSS antenna sat at x -0.400 on a roof that now descends
   from -0.350, so it moves to -0.300, on the roof that is still flat, the
   baseline 0.55 m instead of 0.65; both feet seat on the built glass at
   1.2570 (autonomy-17 carried 1.2650, body-13's crown plus 8 mm of glass that
   Gen 16 made flush, so both feet floated 8 mm: corrected here). The rear
   camera sat at y 0.790 on a Kamm face whose upper section now closes at
   0.66; it moves to 0.600, centered on the face that is there. The flank
   cameras at y 0.945 sit under a belt body-18 did not move and stay; the
   compute plate under the bay floor stays. Everything below is autonomy-17's
   own record. */
...
/* (autonomy-13) Autonomy, Gen 13: autonomy-12 with every mount that is a function of a
   ...
   monitoring, the rings and every mount on a surface that did not move, is
   autonomy-12 to the vertex. Four seats move, each by the amount the
   surface under it moved, and nothing is placed at a coordinate that was
   not first read off the built partner:

     pose antennas   ROOF_SEAT y 1.3300 to 1.2650: body-13's canopy crown is
                     1.2570 and the glass stands 8 mm proud, so the flat the
                     two feet land on is 1.2650 over the same 0.650 m baseline.
     header frame    HDR.pos y 1.288 to 1.223: the canopy at x 0.42 fell
                     exactly 65 mm with the crown.
     flank cameras   y 1.010 to 0.945: the belt came down 65 and the flank
                     at (0.780, 0.945) on body-13 measures the same 0.590 it
                     measured at 1.010 on body-12, so the pads stay at
                     |z| 0.5770 and the camera at 0.5480.
     rear camera     (-2.658, 0.900) to (-2.658, 0.835): the Kamm face now
                     spans y 0.265 to 0.887 and the kamm flap sits at 0.935,
                     so the unit comes down 65 to keep its 12.5 mm standoff
                     and its place under the flap. The washer nozzle rides
                     the same 7.5 mm above it.
   The DMS does not move: the cowl cross member it lands on stays at y 0.896
   because interior-13's hand-wheel mount stands on it too. The compute deck
   does not move: body-13's tail cant rail is unchanged. The radars do not
   move: the nose is held and the tail flank below the waterline is held.
   design/retro-gen12.md section 6 asked for this fork to be planned rather
   than discovered from a floating instance, and it is. */
...
/* Gen 11 autonomy: the same brain, re-seated on a car that is 680 mm
   ...
   the BUILT autonomy-10 against the BUILT body-11:
     ...
     compute      a cold plate at y 0.3895 to 0.4661 on the centerline, which
   ...
   and none of them a judgment call. HANDOFF.md records 13 floating instances
   ...
   did. The driver monitor comes off the header entirely, because a centerline
   occupant 260 mm from the windshield header is a different problem from an
...
/* ── THE COMPUTE POWER BASIS, carried whole ───────────────────────────────
   ...
   34.8 W of deserialization across three lanes is unchanged. The pixel rate
   is unchanged because the imagers are unchanged. The duty split is a
   judgment about SCENES and a narrow car meets the same scenes.
...
/* Duty split over the mixed cycle. The least evidenced input in the module,
   and a judgment about scene statistics rather than a measurement. */
...
/* ── THE MEASURED ENVELOPE ────────────────────────────────────────────────
   ...
   body-11, swept:
     ...
     tail cant rail         cage top 0.7540 at (-1.234, |z| 0.455) and 0.7456
                            ...
                            at the shoe center. Its inboard face is |z| 0.4334
   ...
   wheels-11, swept:
     tire column            |z| 0.6725 to 0.8075, y 0 to 0.7100, and the front
                            tire reaches x 1.8050
...
/* ── WHAT A 1.20 M CAR COSTS THE SENSOR SET ───────────────────────────────
   ...
   2. FRONT CORNER RADAR PAIR: 1.160 m to 0.940 m, a 19.0 percent cut, and
      each unit is now 202.5 mm inboard of its own tire's inner face at
   ...
   body-11 deletes every wheel fairing, so all four tires stand entirely
   ...
   tire column and the built radar mount, the front tire subtends 10.97 to
   ...
   rubber. And the flank camera at (1.010, 0.548) grazing the tire's top inner
   ...
   a tire steers, moves in ride and rotates. The blind wedge became dynamic.
...
export const SYSTEM = {
  id: 'autonomy-23',
  name: 'Autonomy · Gen 23 the sensor duty',
  color: 0x6ea8ff,
  explode: [0, 2.3, 0],
  /* THE ONE PLANE THIS MODULE HAS TO MEET, and it is met rather than
     ...
     and declares it for autonomy-11 to answer. The `compute` part realizes
  ...
  blurb: 'autonomy-22 with the duty argument this module already makes for its compute finally applied to its sensors. Three compute lanes are gated by scene here, one at full rate, one a reduced-rate checker, one in retention, because a cycle that is 58 percent cruise does not need three full stacks; that architecture was then fed by a sensor set running flat out in every scene, 119.4 W of lidar, radar, cameras and inertial, 100 percent of the time. Weighted on this module\'s own published split, 58 cruise, 30 elevated, 12 dense, at 0.40, 0.75 and 1.00, the set averages 0.577 of flat out: 119.4 W becomes 68.9 and the auxDelta goes from -75 to -125.5, which the ladder prices at about 33 miles. The forward lidar and the two forward radars never drop rate; it is the corner units and the surround cameras that do, and any classifier trigger takes the set to full in the 60 ms lane C already specifies. The flank cameras move 30 mm inboard with body-23\'s flank, the same planned fork this module made at Gen 15 and Gen 16.\n\n' + 'autonomy-21 with the fascia suite translated 200 mm aft on the shortened nose, every relative standoff held to the digit: the lidar pucks at (2.13, 0.615, plus and minus 0.30) on a band face at 2.171, the thermal camera at (2.145, 0.53, 0), the imaging radars at 2.090 with their radomes 38.8 mm behind a skin at 2.1648, the corner pockets at 1.940. The band-face contract that survived ten generations because the nose was held does not break, it moves: one translation, two modules, one number. The roof antennas go conformal, 9 mm of foot and dome to a 4 mm patch, returning the 0.0006 m2 of top-band silhouette they owned; sky view, ceramic and ground plane carry.\n\n' + 'autonomy-19 with the network ring\'s rear crossing moved from x -1.12 to -0.98: the center tire reaches -1.0951 at ring height and the carried run stood 25 mm inside it. One coordinate, found by the sweep that opened the rung; everything else carries to the vertex.\n\n' + 'autonomy-18 with one placement moved, the rear camera, from the Kamm face body-19 grows a tail through to the tip of that tail: (-2.658, 0.600) to (-3.258, 0.385), the same standoff class off a cap that moved 600 mm aft, under a light guide that is above the lens now, so the washer nozzle hangs below it, which is the mirror of the reason Gen 10 hung it above. A camera 600 mm further from the axle sees the same road later and the panel owns that number. Nothing else moves; 0 kg.\n\n' + 'autonomy-17 with the aft GNSS antenna at x -0.300 on the roof that is still flat (was -0.400, on a roof that now descends), both antenna feet seated on the flush glass at 1.2570 (they carried 1.2650 and floated 8 mm), and the rear camera at y 0.600 on a Kamm face whose upper section closes at 0.66 (was 0.790). The flank cameras come 27 mm inboard, because at x 0.78 the belt is at 0.895 and the lens at 0.945 sits in a tumblehome body-18 steepened (the skin at its height is 0.523). Nothing else moves; 0 kg.\n\n' + 'autonomy-16 with the compute coolant saddle 20 mm inboard (the handover plane z -0.6560 to -0.6360) because the rear tire of the 1.44 m track stands 3.5 mm inside its old face; the hose ends follow thermal-17\'s ports. Nothing else moves.\n\n' + 'autonomy-16: autonomy-13 with the two flank cameras re-seated on body-15\'s skin, 36 mm inboard, planned as a fork and not discovered from a floating instance; the pads 2 mm off a flank that now measures 0.554 at the pad where it measured 0.590, the separation 1.023 m against 1.096. Nothing else moves and nothing electrical changes: -75 W carried.\n\n' + 'The asymmetric triplex carries whole: one full-rate lane, one diverse checker, one in retention, 192 W typical and the 640 W peak held. Everything else is new, because every sensor on Gen 10 is aimed at a body 680 mm wider than this one. The compute leaves the centerline it shared with the rear occupant and goes behind them; the driver monitor leaves the header, because a tandem driver sits on the axis 260 mm from it; and the two baselines that did not change did not change for reasons that are measurements rather than luck.',
  parts: {
    compute: {
      name: 'Asymmetric triplex compute',
      tagline: 'The catamaran: two hulls flanking the wheel that moved in under the deck, and the sweep finding that a box has no vertices where it hurts.',
      mass: 13.2,
      specs: [
      ...
      fail: [
        ...
        'The tail deck is luggage volume. About 40 liters of the only remaining stowage on a car that already deleted its rear bench goes to a computer, and interior-11 inherits a smaller number than it would like. The alternative volumes were swept and there are none: the flank channel over battery-11\'s sill is filled by hv-11\'s zonal controllers, spine and by-wire feeds and by thermal-11\'s recovery core, and the centerline is occupied by people.',
        'A rear-mounted compute stack is in the rear crush path in a way a centerline one was not. The plate sits at x -1.526 to -1.106 with body-11\'s rear casting starting at x -1.600 and the Kamm face at -2.4427, so there is 0.84 m of structure behind it, but that structure is a tail this generation compressed on an 18.4 degree plan half-angle and it was never sized as a battery-grade intrusion barrier. This module states it rather than pricing it, because rear impact energy is nobody\'s published number on this project.',
        ...
        'The duty split is the softest number in the module. 58 / 30 / 12 is a judgment about scene statistics, not a measurement, and moved with one convention over both ends the published 192 W is 185.0 at a 0.08 dense fraction and 198.4 at 0.16. Carried, and carried with the same warning autonomy-10 attached: the architecture does not change with either, the booked auxDelta does.',
      ],
      explode: [-0.55, 0.35, 0],
    },
    radars: {
      ...
      fail: [
        ...
        'Multipath ghosts come with plausible elevation, and an open wheel adds a new mirror: a wet tire sidewall 200 mm outboard of the corner unit is a specular surface at 77 GHz that no earlier rung had in the near field. Track-level fusion kills any ghost no camera confirms, which is the same answer as before applied to a new cause.',
      ],
      explode: [0.55, 0, 0],
    },
    cameras: {
      ...
      fail: [
        ...
        'A raindrop on a flank lens now has fewer neighboring fields to vote it down and a shorter baseline to the other one; the stack declares the sector degraded rather than guessing, which is the same policy applied to a worse geometry.',
    ...
    dms: {
      ...
      fail: [
        ...
        'Direct low sun up the windshield into a cowl-mounted unit is a worse case than the same sun into a header unit, because the optic now points up rather than down. The bandpass filter narrows the window and dropouts are flagged as monitoring lost, not as driver attentive.',
      ],
      explode: [0.5, -0.3, 0],
    },
    'gnss-imu': {
      ...
      why: 'The pose unit is still the hinge between relative perception and the absolute map, and still the only sensor an attacker can feed from 20,000 km away, so it remains a hint to be verified and never a truth to be obeyed. Redundancy sharpens that stance: a spoofer must bend two antennas and three independent inertial histories into one consistent lie, and consistency is precisely what the voter checks.\n\nThis generation adds one argument the earlier rungs did not need. A tandem is a long thin vehicle, its side area falls from 5.1598 to 4.6293 m2, and design/gen11.md section 9 item 5 says the yaw response changes because of it. A heading estimate good to 0.3 degrees standing still is worth more on a vehicle whose yaw behavior in a gust is less familiar, and it costs nothing extra here, which is the rare case where the package change makes an existing sensor more valuable rather than less.',
      fail: [
...
/* A flat annulus with its axis along +X, centered on the local origin. */
...
/* ── The ring route ───────────────────────────────────────────────────────
   ...
   The loops take the center of it at 0.4815 and 0.4935.
...
const RING = { XF: 1.16, XR: 0.98, R: 0.14, yS: 0.3650, yF: 0.3650, yR: 0.3650 };   /* Gen 21: XR 1.12 to 0.98; the center tire reaches x -1.0951 at ring height */
...
/* ── The header casting ───────────────────────────────────────────────────
   ...
   toward the windshield rather than sitting at its crown: ray-swept on the
...
const HDR = { pos: [0.42, 1.223, 0], tilt: -0.1 };
...
const ROOF_SEAT = [
  /* x, seat y, coax direction in x. 1.2650 = body-13's crown 1.2570 + 8 mm of glass */
  [0.250, 1.2570, -1],    /* Gen 18: on the flush glass; autonomy-17's 1.2650 floated 8 mm */
  [-0.300, 1.2570, +1],   /* Gen 18: forward 100 mm, on the roof that is still flat */
];

/* ── The compute station ──────────────────────────────────────────────────

   CX is the cold plate center and CY its underside. Every absolute
...
/* x station, arm reach in |z|, and the HIGHEST cage vertex anywhere under
   ...
   than at the shoe center. An earlier draft measured a 24 mm box around a
...
export function build() {
  ...
  for (const s of [1, -1]) {
    ...
    bcam.position.set(0.780, 0.945, 0.4536 * s);   /* Gen 23: 30 mm inboard with body-23's flank, which comes from |z| 0.560 to 0.530; at y 0.945 the skin goes 0.523 to 0.493 and this lens keeps the standoff it has had since Gen 18 rather than being buried by a surface that moved without it */   /* Gen 18: 27 mm inboard: at x 0.78 the belt is at y 0.895, the lens at 0.945 is in the tumblehome body-18 steepened, and the skin at its height is |z| 0.523 */   /* Gen 15: 37.4 mm inboard with body-15's flank */
    bp.add(bcam);
    /* Two pads rather than one on the axis, because the camera's own harness
       ...
       at its center stands proud at the bottom and buries its top. */
    const foot = new THREE.Group();
    for (const fx of [1, -1]) {
      const pad = lib.cbox(0.010, 0.020, 0.0080, 0.0009, M.castAlu);
      pad.position.set(0.780 + fx * 0.013, 0.945, s * 0.4826);
      foot.add(pad);
      foot.add(screw([0.780 + fx * 0.013, 0.945, s * 0.4866], [0, 0, s], 0.0024, M.steel));
    }
    bp.add(foot);
    sys.add(bp);
  }
  ...
  /* Rear camera in the Kamm face, GEN 16: 45 mm lower again, because body-16
     shrinks the tail's upper section from x -2.1000 aft (design/gen16.md
  ...
  /* Gen 19: the tip of body-19's tail, the same 32 mm ahead of the last ring
     that the unit has kept since Gen 12; the light guide is at y 0.425 above
     it and the marque above that, so the lens looks out under the lamp */
  rc.position.set(-3.258, 0.385, 0);
  camRear.add(rc);
  /* Washer nozzle BELOW the lens on this rung, and the reason is Gen 10's
     reason mirrored: hung above it lands in the lamp, which is above now. */
  const nozBody = lib.cbox(0.012, 0.009, 0.010, 0.002, M.plasticLt);
  nozBody.position.set(-3.2505, 0.3645, 0.022);
  camRear.add(nozBody);
  const noz = lib.cyl(0.0022, 0.011, M.plasticLt, 8);
  noz.rotation.z = Math.PI / 2;
  noz.position.set(-3.2620, 0.3645, 0.022);
  camRear.add(noz);
  sys.add(camRear);

  /* Thermal imager in the nose face. body-11's skin at (y 0.530, z 0)
     measures x 2.1630 against body-9's 2.1750, so the casing comes back 4 mm
     from Gen 10's station and the germanium window face lands on 2.1580, with
  ...
  th.add(thTail);
  th.position.set(2.1350, 0.53, 0);
  camTherm.add(th);
  sys.add(camTherm);

  /* ── radars ──────────────────────────────────────────────────────────
     The front unit moves 10 mm forward. body-11's bumper beam narrowed to a
     1.00 m span when the car did and its front face measures x 2.0620 at
     y 0.500, so the assembly's rearmost feature, the connector boot at
     x 2.0770, clears it by 15.0 mm and the radome stops at 2.1260 with the nose
     skin 38.8 mm ahead at 2.1648.
     ...
     Front corners: (1.9400, 0.440, plus and minus 0.470), where a 90 by 80 by
     90 mm box holds zero partner vertices, the nose flank is |z| 0.5540 and
     the beam is 122 mm ahead. Rear corners: (-2.1000, 0.650, plus and minus
     0.360), aft of body-11's rear casting which reaches x -2.0450 at |z| up to
     0.560, with the tail flank at |z| 0.4502. */
  const radFront = lib.part('radars', [0.55, 0, 0]);
  const fr = radarUnit(0.150, 0.086, 0.028, 0.036, 9);
  fr.position.set(2.0900, 0.50, 0);
  radFront.add(fr);
  sys.add(radFront);
  ...
  for (const s of [1, -1]) {
    ...
    fu.rotation.y = -0.7 * s;
    fu.position.set(1.9400, 0.440, 0.470 * s);
    cnr.add(fu);
    /* Front corner strut, up to body-11's crash rail. Its underside is a flat
       at y 0.5050 over |z| 0.44 to 0.47 for every x from 1.9000 to 1.9800,
    ...
    fbr.position.set(1.9400, 0.4875, 0.470 * s);
    cnr.add(fbr);
    cnr.add(screw([1.9400, 0.5005, 0.470 * s], [0, 1, 0], 0.0028, M.steel));
    const ru = radarUnit(0.090, 0.060, 0.020, 0.015, 5);
    ru.rotation.y = Math.PI + 0.7 * s;
    ru.position.set(-2.1000, 0.650, 0.360 * s);
    cnr.add(ru);
    /* Rear corner strut, outboard to body-11's tail cone. Its inner surface
       is |z| 0.4502 at x -2.1000 over the whole y 0.60 to 0.68 band, z-rayed
       on the built cone, and it TAPERS: 0.4531 at x -2.0910 and 0.4472 at
       -2.1090, 0.33 m of |z| per meter of x. A 49 mm strut reaching 0.4492 was
       therefore 2.80 mm inside the cone at its aft end over 102 triangle
       pairs even though its center station cleared, which is the same
    ...
    rbr.position.set(-2.1000, 0.650, 0.4200 * s);
    cnr.add(rbr);
    cnr.add(screw([-2.1000, 0.650, 0.4360 * s], [0, 0, s], 0.0026, M.steel));
    sys.add(cnr);
  }

  /* ── lidar ───────────────────────────────────────────────────────────
     Unmoved from autonomy-10 and re-measured rather than carried: the nose
     half-width at (2.1200, 0.615) is |z| 0.5103 and the pucks reach 0.338, so
     the flank is 172 mm away and what binds them is the light band's back
     face at x 2.1640 against a window reaching 2.1585. */
  for (const s of [1, -1]) {
    ...
    /* Ears sideways, not up: the body skin is close above this puck and an
       ear plus a bolt head on the crown left 2.2000 mm of it on Gen 10. */
    for (const sz of [1, -1]) {
    ...
    puck.position.set(2.1200, 0.615, 0.30 * s);
    lid.add(puck);
    sys.add(lid);
  }
  ...
  /* ── the asymmetric triplex, on the tail deck ────────────────────────
     ...
     architecture: lane A a tall vapor-chamber lid, four DRAM packages and a
  ...
  for (const [fx, fz, top] of FEET) {
    ...
    /* Gen 21: two channels a station, one per hull, where one spanned the
       centerline the wheel now owns */
    for (const sc of [1, -1]) {
      const chan = lib.cbox(0.030, 0.014, 0.259, 0.0018, M.castAlu);
      chan.position.set(fx, DECK_BOT - 0.007, sc * 0.2775);
      comp.add(chan);
    }
  }

  /* GEN 21: THE CATAMARAN. The single deck spanned z -0.15 to +0.15 and
     the center tire's crown owns that band to y 0.710 static and 0.790 at
     full jounce: the carried deck stood 30 mm INSIDE the static tire,
     found by the sweep, missed by every vertex probe because a box has no
     vertices in its face interiors. So the deck splits into two hulls
     flanking the wheel at |z| 0.155 to 0.400, directly under their own
     rail stations, same board area, shorter straps, 62 mm to the tire
     face and z-separated from every millimeter of its travel. Lanes A and
     B ride the -z hull, lane C the +z with the holdup bank. */
  for (const sc of [1, -1]) {
    const cold = lib.cbox(0.420, 0.044, 0.245, 0.005, M.castAlu);
    cold.position.set(CX, 0.417 + DY, sc * 0.2775);
    comp.add(cold);
    const cflash = lib.cbox(0.4245, 0.0022, 0.2495, 0.0006, M.castAlu);
    cflash.position.set(CX, 0.4115 + DY, sc * 0.2775);
    comp.add(cflash);
    const deck = lib.cbox(0.404, 0.007, 0.229, 0.0015, M.alu);
    deck.position.set(CX, 0.4425 + DY, sc * 0.2775);
    comp.add(deck);
    const ribs = lib.fins(0.380, 0.0055, 0.205, 9, 0.005, M.castAlu);
    ribs.position.set(CX, 0.3922 + DY, sc * 0.2775);
    comp.add(ribs);
  }

  const LANES = [
    { z: -0.3250, bars: 1, lid: 0.0060, dram: 4, phases: 6, holdup: 0, store: false },
    { z: -0.2300, bars: 2, lid: 0.0028, dram: 2, phases: 3, holdup: 0, store: true },
    { z: 0.2775, bars: 3, lid: 0.0028, dram: 2, phases: 2, holdup: 4, store: false },
  ];
  ...
  /* THE COOLANT RUN, and it is this module's hose because thermal-11 says so:
     ...
     rather than run side by side, which is what a 10 mm lane allows. Third, the saddle that realizes the declared plane therefore
  ...
  /* Two P-clips on the flank run and the saddle that REALIZES the plane
  ...
  for (const [ax, seatY, dir] of ROOF_SEAT) {
    /* Gen 22: conformal. The 9 mm foot-and-dome stack owned the union's
       top band at 0.0006 m2 of silhouette nothing needed; a 4 mm
       automotive conformal patch in the same molding class holds the same
       ceramic over the same ground plane, and the crown keeps its line. */
    const foot = lib.cyl(0.0250, 0.0012, M.alu, 20);
    foot.position.set(ax, seatY + 0.0006, 0);
    ant.add(foot);
    const dome = lib.lathe([
      [0.0000, 0.0000], [0.0215, 0.0000], [0.0220, 0.0005],
      [0.0208, 0.0020], [0.0170, 0.0026], [0.0000, 0.0028],
    ], M.plastic, 16);
    dome.position.set(ax, seatY + 0.0012, 0);
    ant.add(dome);
    ...
    lug2.position.set(ax + dir * 0.0275, seatY + 0.0006, 0);
    ant.add(lug2);
    ant.add(lib.tube([
      [ax + dir * 0.020, seatY + 0.0016, 0.004],
      [ax + dir * 0.032, seatY + 0.0014, 0.010],
      [ax + dir * 0.045, seatY + 0.0012, 0.012],
    ], 0.0026, M.plasticLt, false, 10));
    const grom = lib.cbox(0.014, 0.0060, 0.012, 0.0015, M.plasticLt);
    grom.position.set(ax + dir * 0.049, seatY + 0.0012, 0.012);
    ant.add(grom);
    for (const sz of [1, -1]) ant.add(screw([ax, seatY + 0.0012, sz * 0.0205], [0, 1, 0], 0.0022, M.steel));
  }
  sys.add(ant);
  ...
  /* Gen 21: the IMUs follow their lanes onto the hulls; the old trio at
     the centerline was left floating over the wheel when the lanes moved,
     and the attachment check said so. */
  for (const zi of [-0.3250, -0.2300, 0.2775]) {
