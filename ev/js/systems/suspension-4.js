/* Suspension, generation 4: the active chassis. First variant of a slot
   ...
   box 0.03 x 0.02 x 0.02, centred at EXACTLY each stub point, and runs
...
export const SYSTEM = {
  ...
  parts: {
    'front-structure': {
      ...
      fail: [
        'The Gen 1 field failures carry over unchanged: torn ball joint boots grind joints loose in months, and bush aging shows as inner-shoulder tyre wear before any driver feels it.',
    ...
    'rear-structure': {
      ...
      fail: [
        ...
        'Rear toe is now a software zero plus a mechanical check; an alignment shop that sets it purely mechanically, ignoring the actuator\'s stored centre, will chase its own tail.',
    ...
    'air-springs': {
      ...
      specs: [
        ...
        ['Levelling range', '-20 to +25 mm from nominal'],
        ['Piston area', '61 cm2 front'],
        ['Limp mode', 'Internal bumpstop, 80 km/h flat'],
      ],
      how: 'A rubber sleeve rolls on an aluminium piston; pressure times piston area carries the corner, and the stiffness of the trapped air column sets the rate. Both load and rate scale with pressure, so their ratio, the ride frequency, stays fixed: the property no steel coil has. The fronts are struts, the bellows wrapped around the damper on the Gen 1 coilover line into the same tower mount. The rears stand on the Gen 1 spring-link perch at the same 0.64 motion ratio, keeping the 60 mm of boot floor the divorced layout bought.\n\nGen 1\'s own spec sheet wrote the case for this part. Its rear springs admitted a 25 mm squat under 350 kg of payload, and its flat-ride trick, the 0.1 Hz front-to-rear offset, was tuned true at one design load only. Air holds height to plus or minus 3 mm and holds the offset at every load, so the flat-ride phasing works for one occupant or five. Height then becomes a control variable rather than a consequence: the body drops 20 mm above 110 km/h for roughly 2 percent less consumption, rises 25 mm for a rutted driveway, and kneels at the kerb for loading.',
      why: 'A steel coil is one point on a load-deflection line, chosen the day the prototype was weighed, and the real corner weight then swings 25 percent in service. Air re-chooses the point continuously. The ledger is owned honestly: Gen 1 spent 31 kg on coils and dampers, Gen 4 spends 35 on air, valves, and the supply unit that serves them, and the 4 kg buys levelling, constant frequency, and ride height as a free variable.',
      fail: [
    ...
    'air-supply': {
      ...
      tagline: 'A 3-litre reservoir does the fast work so a 300 W compressor can do the slow work.',
      mass: 7,
      specs: [
      ...
      how: 'The architecture is a reservoir with a compressor behind it. Height changes move air from the 15 bar tank through the valve block into the springs, 40 mm of lift on all four corners in about 6 seconds; the compressor then refills the tank at its leisure. Without the tank the compressor would need better than a kilowatt to feel responsive; with it, 300 W suffices, and 300 W at 48 V is 6.5 A through unremarkable wire where a 12 V design would pull 26.\n\nCompressed air carries water, and ice in a valve block is a dead corner in January, so every litre passes a desiccant bed on the way in and the bed dries itself in the exhaust stream on every vent. The unit lives ahead of the front subframe on the passenger side, low and on rubber isolators, because it runs while the car is parked and levelling, and a compressor heard through a quiet shell at midnight sounds like a fault even when it is doing its job.',
      why: 'One central supply beats four corner pumps because plumbing costs grams per metre while motors cost kilograms each, and beats a bigger compressor because the reservoir converts peak demand into average demand. Sizing for the average is the whole 7 kg staying 7 kg.',
      fail: [
    ...
    'active-dampers': {
      ...
      how: 'The monotube is Gen 1\'s, same bore, same gas backing, same thermal argument: this car pumps 40 percent more heat through its dampers than a 1.6 tonne car, and the single wall still sheds it to the airstream. What changed is the valve. The shim stack was the tune, one digressive curve ground into spring steel; the proportional solenoid is any curve, selected per millisecond across an eight-to-one force span. The control law is skyhook: measure body velocity from the accelerometers and wheel velocity from the travel sensors, firm the valve when damping fights body motion, soften it when damping would only copy the road into the cabin. The 48 V head is what makes the law honest: the solenoid reaches full stroke in 4 ms, where the same coil at 12 V needed four times the current and took three times as long. At 80 km/h, 12 ms is 27 centimetres of road, the difference between reacting at the bump and after it.\n\nThe second input is preview. The forward wedge of autonomy-4\'s camera set maps road height to a few millimetres out to 15 m, and the chassis ECU converts that map into each wheel\'s next half second. A pothole edge arrives with the valve already soft; a crest arrives with it already firm. On the coarse-road benchmark, measured body acceleration drops about 30 percent against Gen 1\'s fixed tune. Heat gets the same treatment: an oil temperature model derates the valve toward firm before the fade Gen 1 could only warn about in its fail list.',
      why: 'Gen 1\'s damper spec was a frozen compromise, taut enough for the motorway, soft enough for town, wrong for each by the margin the other demanded. The valve deletes the compromise, and the fail state is the compromise: de-energised, a spring centres the spool on a fixed orifice cut to Gen 1\'s digressive curve. The worst day of this damper is Gen 1\'s every day, which is the whole fail-safe ladder in one sentence.',
      fail: [
    ...
    steering: {
      ...
      tagline: 'Two complete motor lanes on one rack, and the last mechanical pretence of a column deleted.',
      mass: 11,
      specs: [
      ...
      why: 'Two windings in one slot share a tooth, a thermal path, and a flooded connector shell; two lanes at opposite ends of a housing share almost nothing but the rack itself, which is a forged steel bar with no failure mode short of a crash. Gen 1 wanted this architecture and could not afford the copper: at 12 V a full-authority lane peaks above 90 A and two of them need cable like jumper leads. At 48 V each lane peaks near 23 A, and hv-4 priced the architecture into reach.',
      fail: [
        'Total electrical loss still means no steering, so both zones plus the capacitor bank must together finish a lane change; the bank is sized and tested against exactly that manoeuvre.',
        'Feel is synthesised, so feel remains a calibration liability, carried word for word from Gen 1: a bad release can make good hardware drive badly.',
    ...
    'rear-steer': {
      ...
      fail: [
        'Stuck off-centre is worse than dead, a permanent rear pull, so the screw is deliberately not back-drivable and any fault fires a sprung pin into the centre detent; more than 0.5 degrees of stuck offset invokes a speed limp.',
    ...
    'chassis-ecu': {
      ...
      fail: [
        ...
        'Preview degrades silently in fog, snow, or a dirty windscreen as the camera map thins; skyhook continues on sensors, pre-positioning quietly stops, and the status line says so rather than letting the driver hunt a phantom fault.',
...
function frame(s, withTowers) {
  ...
  /* Inner crossmember, dipped through the middle, same repair as Gen 1 and
     ...
     centre section now runs y 0.192 to 0.232, held up off battery's
...
/* Annulus extruded along z, centred on its own origin. */
...
/* Rolling-lobe air bellows built along Y, centred, plus its top plate. */
...
export function build() {
  ...
  /* ── anti-roll decouplers: Gen 1 bar paths split at centre, rejoined
  ...
  /* lane B, passenger end: the Gen 4 addition, lane A's layout mirrored
     across z so the nut centres sit 0.76 m apart (the 76 cm the content
  ...
  /* centring pin housing, hung below the tube so nothing rises past y 0.22 */
