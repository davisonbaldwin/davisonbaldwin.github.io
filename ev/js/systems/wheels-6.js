/* Wheels and brakes, Gen 5 apex. NAMING: the id and file use the -6 suffix
   ...
   - body-6: its arch clearances were audited against a 185-section tyre
     ...
     110 m3/h per rear arch. Both numbers are held here: the apex tyre
...
export const SYSTEM = {
  ...
  parts: {
    tires: {
      name: 'Tyres',
      tagline: 'The envelope is frozen by body-6\'s arch audit, so the apex tyre spends everything it has inside the same 185 millimetres.',
      mass: 32,
      count: 4,
      specs: [
        ...
        ['Mass per tyre', '8.0 kg including a 0.3 kg sealant and foam liner'],
        ['Maturity', 'Production practice; the steel-free belt is pilot line'],
      ],
      how: 'The first decision was made by a neighbour, not by this module. body-6 audited its arches against the wheels-3 tyre and wrote the numbers down: static faces at |z| 0.9025 and 0.7175, an outside diameter of 0.71 m, 19 mm of clearance at the skirted rear arch once suspension-4 swings the tread corners through plus or minus 3 degrees of rear steer, and 8 mm at the front arch louvers through full lock and full bump. A wider apex tyre would spend those margins and force a body regeneration to buy back what it took. So the envelope is frozen, and the re-specification goes entirely into what is inside it. The carcass becomes a single aramid-hybrid ply where Gen 2 and Gen 3 ran two, the steel belt package is replaced by a high-modulus aramid and carbon hybrid that weighs less and, more usefully, damps better, and the tread runs a functionalised-polymer silica compound whose hysteresis at the 8 to 12 Hz band a rolling tyre actually works in is roughly a fifth lower than the Gen 2 compound this family has carried since 2. Installed rolling resistance falls from 5.2 kg/t to 4.6.\n\nThat 0.6 kg/t is worth putting a number on, because it is the largest single efficiency item in this module. The Gen 5 preset weighs 1,363 kg, which is design/gen5-apex.md\'s 1,369 less the six this module returns, so 0.0006 of rolling resistance coefficient is 8.0 N of drag: 8.0 kJ per kilometre at the wheels, about 2.3 Wh/km out of the pack against a 90 Wh/km budget. Nothing else here comes close. The mass ledger is smaller and less flattering: 8.25 kg per tyre becomes 8.0, and 0.3 kg of that saving is immediately spent again on a puncture sealant and cavity-noise foam liner, which returns the 3 dB Gen 1 bought and then lost, and which matters more now because an in-wheel motor excites the cavity resonance from inside the rim instead of through a bushing. The speed rating moves to Y, not because the car is faster but because a rear tyre that is also a magnet-ring carrier should have its casing qualified above every duty it will ever see rather than at it.',
      why: 'Three generations of this module have argued that the tyre is where the physics lives, and the apex generation is the one that has to accept a constraint it did not set. body-6 spent its clearances on a shape that is worth 0.130 of drag, and taking them back for a wider contact patch would be a trade between two neighbours with the same owner. Freezing the envelope and rebuilding the casing inside it is the honest move: it buys 2.3 Wh/km without asking anyone else to give anything up.',
      fail: [
        'The ride bill compounds again, exactly as wheels-3 predicted. A stiffer single-ply carcass at 3.4 bar passes sharp inputs almost straight through, and suspension-4\'s active dampers answer road inputs below about 25 Hz. Above that the tyre sidewall is the only spring in the system, and at the rear it is protecting a 1.0 mm airgap.',
        'A steel-free belt has a shorter fleet history than anything else in this module and its repair rules are different: a plug that is routine in a steel-belted casing is not qualified here, so a rear puncture is a tyre replacement rather than a repair, and the sealant liner exists to make that a scheduled event rather than a roadside one.',
        'The apex compound trades wet grip for hysteresis at the margins. The lateral limit holds near 0.85 g dry, stability control still covers the gap, and the honest sentence is that a tyre optimised this hard for rolling loss is a tyre that gives up something in standing water, which is why the aquaplaning threshold is a published number rather than a footnote.',
      ],
      explode: [0, 0, 0.85],
    },
    'front-wheels': {
      ...
      fail: [
        ...
        'Carbon creeps under sustained clamp load and aluminium does not, so a wheel nut torqued into polymer loses preload over years. The co-cured titanium insert exists for that reason and moves the risk rather than deleting it: the new watch item is the titanium-to-CFRP co-cure itself, inspected by tap test at every tyre change and instrumented with load-indicating washers under the nuts.',
    ...
    covers: {
      ...
      fail: [
        ...
        'The rear cover still rides at the 0.97 explode vector rather than the corner\'s 0.85, because drivetrain-6\'s ring would otherwise pass through the blade face during teardown. That is a modelling convenience with a real-world echo: on this car the cover cannot be removed with the motor ring in place, so a cover service is a corner service.',
      ],
      explode: [0, 0, 0.97],
    },
    discs: {
      ...
      count: 2,
      specs: [
        ['Size', '365 x 34 mm C/SiC (wheels-3: 355 x 28 grey iron)'],
      ...
      fail: [
        ...
        'C/SiC wears by oxidation of its carbon phase at high temperature rather than by abrasion, so disc life is counted in thermal cycles rather than kilometres, and a disc that looks perfect can be at the end of its life. Mass loss is tracked at service against a serialized as-built figure, and the disc is not machinable: there is no skim, only replacement.',
...
/* ── Geometry ──
   ...
   wheel centre (+-1.45, 0.355, +-0.81), z along the axle, s = sign(world z)
   ...
   Key world planes used below:
     |z| 0.7175  tyre inner face (body-6 audited)
     ...
     |z| 0.9025  tyre outer face (body-6 audited) */
...
/* One ring of pitched vane blades. The blade is drawn span-along-local-y,
   ...
   |z| 0.808 to 0.936 (the 3 and 9 o'clock blades tip 33 mm past the tyre
...
/* ── Rear wheel ──
   ...
   The centre web is new this generation: a hub pilot landing on
...
/* ── Covers, static half ──
   ...
   wall and its outer face at r 0.084, five millimetres inside the
...
/* ── Front discs ──
   ...
   it: the upper sphere (centre radial 0.210, radius 0.024, inner edge
   0.186) clears by 3.5 mm, the lower one (centre radial 0.120, outer edge
   0.144) by 1.0 mm.
...
function caliperPart(x, z) {
  ...
  /* Six pistons, three a side, crowns landing on the friction ring faces
     ...
     separately). The row is centred at radial 0.162 with plus and minus
  ...
  /* Two-lug bracket back to the upright. It runs at |z| 0.716 to 0.719,
     ...
     from the wheel centre so it clears the disc hat bell, which is at
...
/* ── Park brake, static half ──
   ...
   shoe web threads r 0.073 to 0.089, a millimetre outside the static hub
   housing at 0.072 and a millimetre inside the drum band at 0.090, which
...
/* ── Brake-by-wire unit ──
   ...
   side legs at |z| 0.640 to 0.690, and inboard of the tyre faces. Both
