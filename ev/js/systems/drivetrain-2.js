...
export const SYSTEM = {
  ...
  parts: {
    ...
    'rotor-disc': {
      ...
      mass: 5,
      specs: [
        ['Magnets', 'NdFeB, 20 poles, through-magnetized'],
      ...
      fail: [
        'Hoop fiber is the containment case: a steel rotor fails by yielding, a wound hoop fails by unraveling, so burst margin is proven by overspeed-to-failure tests and the stators double as containment rings.',
        'Sustained heat soak, a mountain climb towing, drives magnet temperature toward the knee where fault currents could demagnetize; the inverter derate protects the magnets, not the copper, same logic as Gen 1 but with less margin behind it.',
    ...
    halfshafts: {
      ...
      fail: [
        ...
        'Curb-strike overload protection still depends on the traction controller cutting torque faster than the shaft reaches its ultimate rating; the retuned damping loop had to prove it kept that reaction time.',
      ],
      explode: [0, 0, 0.75],
    },
    inverter: {
...
/* Rear unit axes in the x-y plane, motor axis along z (pancake disc).
   A2 = motor axis, B2 = intermediate (counter) axis, C2 = ring gear,
   differential and halfshaft axis at wheel-center height. */
const A2 = [-1.48, 0.40];
const B2 = [-1.505, 0.458];
const C2 = [-1.45, P.wheelY];     // 0.355
const GZ2 = -0.16;                // gear case center plane, z

/* Axial stack along z: inboard stator, rotor, outboard stator. */
const Z_ROT = 0.040;              // rotor disc center
const Z_ST_OUT = 0.072;           // outboard stator center plane
const Z_ST_IN = 0.008;            // inboard stator center plane
...
/* One stator disc: thin yoke ring, 24 SMC teeth, copper coil ring visible
   ...
   rotor), dir = -1 the inboard disc (teeth face +z). Group is centered on
