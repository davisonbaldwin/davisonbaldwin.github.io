/* Wheels and brakes, generation 2: narrow high-pressure low-loss corners.
   Delta against wheels.js: 185/50 R21 tires on carbon-hybrid 21s, fully
...
export const SYSTEM = {
  ...
  color: 0xd0d6db,
  explode: [0, 0, 0],
  blurb: 'Gen 1 gave regen the everyday stops. Gen 2 attacks what was left: rolling loss, pumping drag, residual friction, and rust. Narrow high-pressure tires, sealed carbon-hybrid corners, rear drums: 96 kg against 133, about 15 Wh/km returned.',
  parts: {
    tires: {
      name: 'Tires',
      ...
      how: 'A tire’s contact patch area is roughly load over pressure; what the designer actually controls is its shape. The Gen 1 235/50 R19 spent that area wide and short. This casing spends it long and narrow: 185 mm of section against 235, on a 21 inch rim, rolling radius held within a percent of Gen 1 so gearing, speedometer, and ride height carry over. Patch length is what straight-line braking and hydroplaning care about, because both depend on how long each tread element stays in contact; width is what rolling loss and aero care about. Keeping the length while surrendering the width is the entire trick. The supporting moves: cold pressure rises from 2.9 to 3.4 bar to cut carcass deflection, the tread band a fifth narrower puts a fifth less rubber through the flex cycle every revolution, and the sidewall drops from 117 to 92 mm, because at 3.4 bar a short sidewall barely bulges under load, and rubber that does not deform does not lose energy. Installed Crr lands at 5.2 kg/t against the Gen 1 tire’s 7.2 as mounted; its drum label said 6.5, and coarse asphalt has always added the difference.\n\nThe budget math is why this tire exists. At 2,100 kg, each 0.001 of Crr is about 20 N of drag, 5.7 Wh per km at the wheels, roughly 6 Wh/km out of the pack. Going from 0.0072 to 0.0052 returns about 12 Wh/km, close to 8% of highway consumption, before counting the 50 mm of exposed frontal width removed at each front wheel, worth another 1.5 Wh/km or so. The payment is lateral: cornering stiffness tracks section width, and the steady-state limit drops from about 0.92 g to about 0.85 g. Braking distance barely moves, because it runs on patch length and vertical load, and both are unchanged.',
      why: 'The Gen 1 tire bought a sports-sedan lateral limit and paid for it on every meter it ever rolled. Priced honestly, the last 0.07 g of cornering cost about 12 Wh/km and was collected by almost nobody. Gen 2 moves the spend to where the car actually lives, rolling straight at highway speed, and lets stability control manage the smaller margin.',
      fail: [
    ...
    rims: {
      ...
      tagline: 'Carbon where the radius is, aluminum where the abuse is.',
      mass: 31,
      count: 4,
      specs: [
      ...
      fail: [
        ...
        'The bond line is the life-limiting feature; adhesive plus bolts is belt and suspenders precisely because either alone carries an open fatigue question at 300,000 km.',
    ...
    covers: {
      ...
      fail: [
        'The flush face hides the wheel entirely; owners who removed Gen 1 covers for looks will remove these too, and the quoted consumption still assumes they are installed.',
        'Inboard-fed disc cooling is sized for the road duty cycle, not for track use; two hot laps will find the limit.',
        'A curbed or delaminated cover cannot be clipped back on at the roadside; it is a bolted, sealed part with a lead time.',
      ],
      explode: [0, 0, 1.45],
    },
    discs: {
    ...
    drums: {
      ...
      specs: [
        ...
        ['Shell', 'Aluminum, cast-in iron liner'],
      ...
      fail: [
        ...
        'Deep water can be drawn past the seal as a hot drum cools; the first apply afterward is weak until the shoes dry, the classic drum vice reduced but not deleted.',
...
/* ── Geometry ──
   Corner-local frame matches the reference: origin at the wheel center,
   z along the axle, s = sign(world z) so +s is outboard. Tire, rim, cover,
...
function rimPart(s) {
  ...
  g.add(barrel);
  /* Forged aluminum spider: five slim spokes, bonded ring, hub cap. */
  for (let i = 0; i < 5; i++) {
...
/* Front fixed calipers at 9 o'clock, behind the axle, clear of the
   steering arc. Rides above the axle centerline like the reference so the
