...
export const SYSTEM = {
  ...
  color: 0xc0a2ec,
  explode: [0, -0.62, 0],
  blurb: 'Double wishbones ahead, five links per side behind, and a steering rack no shaft touches. The 164 kg that decides how 2.2 tonnes reads the road.',
  parts: {
    'front-subframe': {
      name: 'Front subframe',
      tagline: 'An aluminium cradle that collects every front corner load and hands it to the body through rubber.',
      mass: 24,
      specs: [
    ...
    coilovers: {
      ...
      tagline: 'A spring that carries half a tonne, and a damper whose real job is turning motion into heat.',
      mass: 18,
      count: 2,
      specs: [
    ...
    'arb-front': {
      ...
      how: 'When both wheels rise together the bar simply rotates in its bushes and adds nothing. When the body rolls, one end arm swings up and the other down, and the centre section is placed in pure torsion; the torque it develops pushes the loaded side back up. It is a spring that acts only on the difference between the two wheels, which is why it can stiffen roll without stiffening ride. The tube is hollow because torsional stress lives at the surface: deleting the core cuts 40 percent of the mass for about 5 percent of the rate.\n\nSizing starts from the roll budget. On springs alone this car would roll about 5.6 degrees at 0.8 g; the target is under 3.5, and the bars supply the difference, split deliberately unevenly: the front pair of wheels carries 58 percent of total roll stiffness on only 48 percent of the mass.',
      why: 'Roll stiffness distribution is the balance knob of the whole car. The axle carrying the larger share works its outside tyre harder and saturates first, so a front-biased split makes the limit behaviour understeer: stable, undramatic, recoverable by lifting. The front bar is set stiff to guarantee that character, and the smaller rear bar trims it.',
      fail: [
    ...
    'rear-subframe': {
      name: 'Rear subframe',
      tagline: 'A welded aluminium frame that gives ten links and a drive unit one stiff place to stand.',
      mass: 28,
      specs: [
      ...
      how: 'Five links per side means ten inner pivots, each seeing up to 9 kN, and each needing an abutment that does not move. The rails and crossmembers box the loads in plan; the cast towers rise above the rails to carry the upper camber links at the height their geometry demands. Stiffness here is not vanity: the rear elastokinematics assume the pivots stay put, and the toe behaviour the links are tuned for lives in bushing deflections of a few tenths of a millimetre. A frame that flexed a whole millimetre would erase the tuning.\n\nThe body mounts are hydraulic: rubber bushes with internal glycol chambers and an orifice between them. At small amplitudes they are soft and isolate road noise like plain rubber; at large amplitudes the fluid is forced through the orifice and they damp. That second behaviour matters because the rear drive unit hangs on this frame, and one-pedal driving swings its torque through zero dozens of times per journey. The hydraulic mounts kill the driveline shuffle those reversals would otherwise feed into the floor.',
    ...
    'rear-springs': {
      ...
      why: 'Divorcing the spring from the damper buys 60 mm of boot floor depth, because the fat coil tucks under the floor while only the slim damper rises into the wheelhouse. On an EV the boot is one of the few packaging wins left to give the customer; the suspension pays for it with one extra perch.',
      fail: [
...
/* Local stand-in for lib.mirrorZ. THREE's Object3D.copy JSON-serialises
...
/* Subframe ladder, s = +1 front / -1 rear. Rails start at |x| 1.31 to stay
   ...
   housing, stators and rotor disc. The centre section now runs y 0.192 to
...
/* Annulus extruded along z, centred on its own origin: a bore drawn as a
...
export function build() {
  ...
  /* steering: rack + belt actuator + tie rods + by-wire column stub.
     Rack centreline at y 0.36: the ball nut (r 0.046) is the deepest piece
