/* GEN 21: THE CENTER RING. drivetrain-17 with the rear pair become one:
   the +z corner's whole arrangement slides 0.72 inboard onto the
   centerline (the contract flange was always 90 mm inboard of its wheel
   center, and the center wheel keeps that number at z -0.0900 off z 0),
   the ring uprated 1.4 in copper and cooling, the bearing 1.3 in
   complement, one adapter ring to suspension-21's fork plate, one loop
   set, zero rear vectoring. The annulus is declared SIGNED for the first
   time (z -0.09..-0.01, no mirror; tools/interfaces.js gained the
   reading). Every rear |z| in the carried text below describes corners at
   0.72 that no longer exist; subtract 0.72 and drop the mirror to read it
   at the centerline. design/gen21.md is the brief.
...
export const SYSTEM = {
  id: 'drivetrain-21',
  name: 'Drive units · Gen 21 the center ring',
  color: 0xe8a24c,
  ...
  /* GEN 21: the band slides to the centerline with the ring and has no
     mirror, so it is declared SIGNED (raw z, not |z|): the tool gained
     that capability for this rung and every earlier declaration reads as
     before. */
  annulus: { rIn: 0.155, rOut: 0.235, zIn: -0.09, zOut: -0.01, signed: true },
  explode: [0, 0, 0],
  blurb: 'drivetrain-17 with the rear pair become one: the +z corner\'s whole ring arrangement, rotor on the flange, stator behind it, bearing unit, inverter and loops, slides 0.72 inboard onto the centerline as one rigid body, because wheels-21\'s center wheel carries its contract flange at z -0.0900 off a center at z 0, the same 90 mm inboard of center the corner wheels always had. Every intra-corner clearance therefore carries by construction, and the annulus is re-declared as a signed band (z -0.09 to -0.01, no mirror; the checker gained that reading for this rung). The one ring carries 1.4 of a corner ring\'s torque, 994 Nm peak, by copper fill and the doubled cooling one ring inherits from a loop pair that served two, stated as undrawn mass per the ledger rule; the bearing uprates 1.3 for the doubled load; a 30.5 mm adapter ring bridges the carrier flange to suspension-21\'s fork plate, because the upright the carrier used to bolt left with the corner. Rear yaw vectoring is gone with the pair, and the coordinator\'s panel says what that means plainly: the trike\'s yaw actuators are the front axle\'s steering and brakes, full stop. 94.2 kg against 117.3, the front unit, disconnect, inverter and halfshafts carried to the vertex.\n\n' + 'drivetrain-9 with the rear rings 20 mm inboard and the front halfshafts 20 mm shorter, for wheels-17\'s 1.44 m track; the annulus the rings occupy is declared for the checker. Nothing else moves.\n\n' + 'drivetrain-9: drivetrain-7 with one dimension changed. The front track came in 70 mm per side, so each front halfshaft loses 70 mm of bar and its outboard joint face lands on wheels-9\'s hub face at |z| 0.6925 instead of overrunning it. That is worth 0.42 kg and it costs 19 percent more articulation angle for the same wheel travel, both computed rather than claimed. The reason it gets a generation number is not the geometry: this interface was found 10 mm short at Gen 8 and 70 mm long at Gen 9, wrong twice running in opposite directions, and the fix it needed was an assertion rather than a third manual correction. This module now declares that plane as an interface and tools/check-interfaces.sh proves it against both halves\' built meshes.',
  ...
  parts: {
    'stator-rings': {
      ...
      tagline: 'One ring where the pair stood, at 1.4 of a corner ring\'s torque: the geometry is the pair\'s to the vertex, the copper is not, and the cooling it inherits is double.',
      mass: 18.9,
      count: 1,
      specs: [
    ...
    'rotor-rings': {
      ...
      tagline: 'One rotor on the centerline flange, its Halbach array carried to the segment; the 32 bolts\' worth of history below describes corners that no longer exist.',
      mass: 9.8,
      count: 1,
      specs: [
    ...
    'hub-bearings': {
      ...
      tagline: 'One unit at double the load on a spindle instead of an upright, with the 30.5 mm adapter that bridges a joint whose other half left with the corner.',
      mass: 11.7,
      count: 1,
      specs: [
    ...
    'corner-inverters': {
      ...
      tagline: 'One inverter at 1.4 of a corner\'s current, in the column the corner pair proved, and the low-load carrier trick matters more with one ring than it did with two.',
      mass: 3.5,
      count: 1,
      specs: [
    ...
    'service-loops': {
      ...
      tagline: 'One dual loop to the centerline, riding the swingarm it flexes with, and the whole fatigue case got easier: a trailing arm\'s arc is gentler than a five-link corner\'s.',
      mass: 3.0,
      count: 1,
      specs: [
    ...
    'tv-controller': {
      ...
      tagline: 'The vectoring half of the name retires: one centered wheel makes no couple, and what remains is the connection arbiter and the honest sentence about where yaw now lives.',
      mass: 2,
      specs: [
...
function tvControllerPart() {
  ...
  /* GEN 21: the box rides 100 mm outboard to z -0.20: its old station's
     +z half stood inside the center tire's band (the checker read 22.5 mm
     into the wheel), and the penthouse still pins x. */
  const body = lib.cbox(0.14, 0.045, 0.16, 0.0055, M.castAlu);
  body.position.set(-1.66, 0.43, -0.20);
  part.add(body);
  ...
  part.add(lib.fins(0.116, 0.009, 0.140, 11, 0.0035, M.castAlu)
    .translateX(-1.66).translateY(0.4570).translateZ(-0.20));
  ...
  pcb.position.set(-1.66, 0.4045, -0.20);
  part.add(pcb);
  /* THE EARS LEAVE THROUGH THE z FACES, NOT THE x ONES, and that is a fix
     ...
     -1.730..-1.590. */
  for (const [ex, ez] of [[-1.7000, -0.2920], [-1.6200, -0.2920],
                          [-1.7000, -0.1080], [-1.6200, -0.1080]]) {
  ...
  for (const [cx, cz] of [[-1.62, -0.25], [-1.70, -0.15]]) {
  ...
  /* corner harnesses to the service-loop brackets at (-1.36, 0.425, ±0.625).
     ...
     measures it again. */
  /* GEN 21: one corner harness where two were, to the center corner's
     loop bracket at its translated station; the wheel it serves is the
     wheel the box moved around. */
  part.add(lib.tube([
    [-1.700, 0.462, -0.150],
    [-1.660, 0.430, -0.128],
    [-1.560, 0.410, -0.124],
...
export function build() {
  ...
  /* GEN 21: THE CENTER CORNER IS THE +z REAR CORNER TRANSLATED 0.72
     INBOARD, WHOLE. The arithmetic that allows a pure translation: the
     old contract flange at |z| 0.6300 is the wheel center 0.72 less
     0.090, and wheels-21's center wheel puts its flange at z -0.0900 off
     a center at z 0, the same 0.090 in the same direction. So the ring
     pair's whole arrangement (rotor on the flange, stator behind it, the
     bearing unit, the inverter, the loops and every clearance among them
     and to the drum, backplate and gearmotor, which wheels-21 translated
     identically) slides to the centerline as one rigid body and lands on
     the new wheel exactly. One corner, not two: the -z mirror never
     existed at the centerline. */
  const center = new THREE.Group();
  center.add(rotorRingPart(1));
  center.add(statorRingPart(1));
  center.add(hubBearingPart(1));
  center.add(cornerInverterPart(1));
  center.add(serviceLoopPart(1));
  center.position.z = -REAR_Z;
  sys.add(center);

  /* the carrier adapter: the old carrier flange face lands at z -0.1005
     after the slide, and the mate it bolted (the deleted upright's barrel)
     left with the corner; suspension-21's fork plate stands at -0.131. A
     30.5 mm spacer ring bridges the two, four bolts through. */
  /* r 0.104 to 0.126: outside the park backplate's 0.100, inside the
     carrier neck's 0.132, with the re-clocked gearmotor at r 0.081 in its
     bore; a first draft at r 0.056..0.086 shared the backplate's own
     radii and the checker said so at 13.0 mm */
  /* four lugs, not a ring: a full ring at these radii reaches y 0.481 at
     the top, where the house's foot and the fork boss own the plane, so
     the torque path clocks to 45, 135, 225 and 315 degrees and leaves the
     crown sector open */
  const adapter = lib.part('hub-bearings', [0, 0, -0.78]);
  for (let i = 0; i < 4; i++) {
    const a = (i / 4) * Math.PI * 2 + Math.PI / 4;
    const lug = lib.cbox(0.034, 0.030, 0.025, 0.004, M.castAlu);
    lug.position.set(RX + Math.cos(a) * 0.115, WY + Math.sin(a) * 0.115, -0.11875);
    lug.rotation.z = a + Math.PI / 2;
    adapter.add(lug);
    const b = zc(lib.cyl(0.007, 0.010, M.steel, 6));
    b.position.set(RX + Math.cos(a) * 0.115, WY + Math.sin(a) * 0.115, -0.136);
    adapter.add(b);
  }
  sys.add(adapter);
