/* Suspension, generation 9: the narrow-track front. suspension-4 carried
   ...
   reading partner prose, all reported in the panels:
     ...
     3. The Gen 4 by-wire stub points at |z| 0.70 are now inside the
        wheels at BOTH ends. Contract honoured, defect reported, amendment
        named. See the 48v-feeds panel.

   BINDING interface contract with hv-4 (design/gen4.md, restated binding
   in design/gen9.md): connector bosses 30 x 20 x 20 mm centred at EXACTLY
   ...
   centred on the contract points to the micrometre. The bosses here sit
   on those points.

   Hard points this module CHANGES, for body-9 and drivetrain-7 to honour:
...
const FZ = 0.74;        // front wheel centre |z|, was 0.81
const RZ = 0.74;        // rear wheel centre |z|, carried from Gen 7
  ...
  parts: {
    ...
    'front-knuckle': {
      ...
      fail: [
        ...
        'Moving the body inboard lengthens the overhang from the body plane to the wheel centre from 70 to 157 mm, so the bearing sees more moment for the same wheel load. The bearing is sized for it and the section is not free: that is part of the two kilograms the front structure gained.',
      ],
    ...
    'rear-structure': {
      ...
      mass: 55,
      specs: [
        ['Wheel centre', '(-1.45, 0.355, +/-0.74), carried from Gen 7'],
      ...
      fail: [
        ...
        'The upright now bolts to a face drivetrain-7 owns, at |z| 0.6395. Any future rear motor that moves that face by a millimetre moves this suspension, and the two modules are now geometrically married in a way the annulus contract alone did not make them.',
      ],
      explode: [-0.15, -0.45, 0],
    },
    'air-springs': {
      ...
      fail: [
        ...
        'The rear seat now sits 61 mm closer to the car centreline on a link that is 100 mm shorter, so the same spring force reacts through a shorter lever into the same bush. Bush load at the rear inner pivot rises about 8 percent for the same wheel load.',
    ...
    'air-supply': {
      ...
      tagline: 'The unit is untouched by the track. One of its four lines was not, and it was found inside a steered tyre.',
      mass: 7,
      specs: [
      ...
      how: 'The reservoir and compressor sit where Gen 4 put them, ahead of the front subframe on the passenger side, on rubber, because the unit runs while the car is parked and levelling. The architecture argument is unchanged: a 3 litre tank at 15 bar converts a peak demand into an average one, so 40 mm of lift on four corners takes 6 seconds from stored air while a 300 W compressor refills at leisure, and 300 W at 48 V is 6.5 A through unremarkable wire.\n\nWhat did change is the plumbing, and one line changed for a reason the first draft of this module missed. The front spring seats moved 70 mm inboard and the rear seats 61 mm, so all four supply lines are re-routed and two of them are now shorter; the cross-car line to the driver-side front spring drops from 1.42 m to 1.28. The fourth line is the rocker trunk to the rear passenger spring, and suspension-4 ran it at |z| 0.62 to 0.63 through the front wheelhouse, which was 45 mm clear of a wheel centred on 0.81. Pulling it in by 20 mm was not enough. Sweeping wheels-9\'s built front corner about the kingpin axis, the swept solid at the commanded 14 degrees of lock reaches |z| 0.5844 at x 1.10 and 0.5600 at x 1.30, and the line as drawn stood 25.4 mm inside the tyre. It now runs at 0.505 to 0.522 through that band and returns to 0.61 through the cabin, because interior-6 holds seat-rail material out to |z| 0.543 under the floor and a line pulled in for its whole length would have traded a tyre for a seat rail. A pressurised line inside a steered tyre is not a clearance question, it is the line, and it was found by sweeping rather than by looking, because at zero lock it was clear.\n\nLine volume is part of the lift-speed budget, and the shorter runs give back about 4 percent of the 6 second figure, which is inside the rounding and is not claimed as an improvement.',
    ...
    steering: {
      ...
      specs: [
        ...
        ['Lane centres', '+/-0.38, separation 0.76 m unchanged'],
      ...
      fail: [
        'Total electrical loss is still no steering, so both zones plus the capacitor bank must still finish a lane change, and the bank is still sized against exactly that manoeuvre.',
        'Feel is still synthesised and still a calibration liability, carried word for word from Gen 4.',
        'A shorter rack with the same lane separation leaves less bar between the outer nut and the rod end, so the rack\'s own bending compliance under a one-wheel kerb strike rises about 9 percent. It is a comfort and precision number rather than a strength one, and it is measured rather than estimated.',
    ...
    'rear-steer': {
      ...
      specs: [
        ...
        ['Fail', 'Sprung and pinned to centre'],
        ['Maturity', 'Pilot line'],
      ],
      ...
      fail: [
        'Stuck off-centre is still worse than dead, still not back-drivable, still pinned by a sprung detent, still speed-limped past 0.5 degrees of stuck offset.',
    ...
    '48v-feeds': {
      ...
      tagline: 'The contract is honoured exactly, and honouring it exactly is what proves it has to change.',
      mass: 1,
      count: 4,
      specs: [
        ['Boss centres', '(+/-1.30, 0.36, +/-0.70), contract exact'],
...
/* ── Detail vocabulary ──────────────────────────────────────────────────
   ...
   members that cross it to a tenth of a millimetre. So no detail feature
...
/* Bush: the arm's own forged eye, the rubber, and the inner crush tube
   ...
   "spanning 0.32 to 0.36, ten millimetres outside" a drive zone this module
   itself narrowed to |z| 0.31, so a 1.32 r eye grown around the old rubber
   would have put 6.4 mm of aluminium inside the zone and made the panel
...
/* Plain ball: kept for the FRONT corner joints only, where the published
   ...
   millimetre and a boot would add an eighth member to a list of seven. */
...
/* Ball joint with the hardware a real one carries: a housing, a moulded
...
/* A fastener head seated on a face, axis along n. lib.fastener puts its
   ...
   the built mesh, never its centre and never its datum. */
...
/* A convoluted gaiter along local Y, centred, for a rack or an actuator
...
/* A P-clip: the strap round a line plus the foot it is screwed through.
   A pressurised line that floats through a wheelhouse is a line nobody
   routed, and this car has eleven metres of them. */
...
/* An anti-roll bar bush: the rubber D-bush, the strap that clamps it and
   ...
   to a tenth of a millimetre and neither one moves; this is the hardware
...
function frame(s, withTowers) {
  ...
  /* Rails and crossmembers are hollow extrusions with a chamfered corner.
     ...
     a solid bar. Ends and centrelines are the Gen 4 ones to the millimetre:
  ...
  for (const zs of [-1, 1]) {
    /* Body mount: the bush sits in a machined seat on the rail top under a
       ...
       above. The four centres are untouched at (s*1.34, s*1.72, 0.30,
...
function frontUpright() {
  ...
  /* THE BODY HAD NO BEARING BORE, AND THE HALFSHAFT WENT THROUGH THE SOLID.
     It was one 85 by 360 by 32 mm chamfered box centred on x 1.47, so its
  ...
  /* THE OUTBOARD END OPENS TO r 0.055 BECAUSE THE HUB IS DRAWN TWICE.
     ...
     to 0.0520 over |z| 0.6825 to 0.6925, ten millimetres thick, and it is
     ...
     halfshaft goes into the CENTRE of the hub and the wheel clamps on the
...
/* Rolling-lobe air bellows built along Y, centred on its own origin, with
   the four things the old zigzag lathe left off: the ALUMINIUM PISTON the
...
export function build() {
  ...
  /* The upper inner pivots pick up on body-9's front casting rather than on
     ...
     is made of. The pivot centres are the published (1.33 and 1.65, 0.55,
     +/-0.34) to the millimetre; only the hardware around them is new. */
  ...
  /* Five-link rods, forged rather than turned: a section deeper than it is
     ...
     outer joint centres are exactly where the rate arithmetic put them, and
  ...
  /* Rocker trunk line to the rear passenger spring. suspension-4 ran this at
     |z| 0.62 to 0.63 past a front wheel centred on 0.81. The wheel came in
  ...
  /* On the ears, outboard of the cast body's 0.088 by 0.058 half extents.
     A torx head of r 0.0055 carries a flange of r 0.00671, so a centre at
