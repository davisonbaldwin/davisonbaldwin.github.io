/* Autonomy, Gen 12: autonomy-11 with ONE mount moved, and the mount is the
   rear camera. Everything else, the triplex compute, the radar ring, the
   cameras' shared header, the monitoring, the rings and every other mount,
   is autonomy-11 to the vertex, because Gen 12 does not ask the sensing
   architecture for anything new. It asks it to fit a car whose tail moved.

   WHY IT EXISTS. body-12 takes the tail from x -2.4427 to -2.6900
   (design/gen12.md), and autonomy-11's rear camera was seated against
   body-11's tail face by measurement: unit at (-2.410, 0.900, 0), lens
   12.5 mm off a skin at -2.4405. On the gen12 preset that skin is 248 mm
   further aft and tools/check-interfaces.sh reports the instance FLOATING,
   nearest surface 92.1 mm away, which is the exact failure mode this
   module's own flank-camera paragraph documents against autonomy-10 and
   the re-seat condition design/gen12.md section 6 ordered checked. The
   unit moves to (-2.658, 0.900, 0) with the same 12.5 mm lens standoff
   off the new face, measured at -2.688, and the washer nozzle rides the
   same 7.5 mm above and behind it that it always has.

   This is drivetrain-9's precedent applied one module over: a halfshaft
   length is a function of the track, and a camera mount is a function of
   the tail. The fork is the honest cost of a preset carrying a module
   whose geometry is a function of a partner that changed, and the
   alternative, moving autonomy-11's own camera, would strand it 248 mm
   inside Gen 11's tail on the rung that owns it. */
...
export function build() {
  ...
  /* Rear camera in the Kamm face, THE ONE MOVED MOUNT IN THIS FORK.
     body-12's tail face measures x -2.688 at (y 0.900, z 0) and its kamm
     flap spans x -2.6149 to -2.4300 at y 0.9582 to 1.0207, so the unit
     sits under the flap and behind nothing, with the same 12.5 mm lens
  ...
  nozBody.position.set(-2.6505, 0.9205, 0.022);
  camRear.add(nozBody);
  const noz = lib.cyl(0.0022, 0.011, M.plasticLt, 8);
  noz.rotation.z = Math.PI / 2;
  noz.position.set(-2.6620, 0.9205, 0.022);
  camRear.add(noz);
  sys.add(camRear);
