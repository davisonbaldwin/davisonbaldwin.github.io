...
export function build() {
  ...
  /* The fluid keeps battery-6's plan, |z| 0.670 and 0.595: the retaining
     ...
     wheels-9's front tire already reaches |z| 0.6625 over x 1.095 to 1.353
  ...
  for (const zs of [-1, 1]) {
    ...
    /* Riser take-offs: the header collects from the inter-brick gaps, so it
       ...
       tire, so the forward gap vents aft under the lid instead. */
...
/* ── Verification notes (measured off partner build geometry, not prose) ──
   ...
   x 1.095 to 1.268 sits inside wheels-9's front tire, whose inner face
   ...
   that a front tire at |z| 0.6625 and a pack at 0.700 cannot both be right.
