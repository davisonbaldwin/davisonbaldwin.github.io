/* Gen 11 battery: battery-10's pack, unmoved, with its rocker bond rail and
   ...
   |z| 0.7800, already inside the tyre column at 0.6725 to 0.8075. So the
   ...
   places them at 0, plus and minus 66.68 mm about the section centre. The
   outer two therefore reach y 0.098 from that centre, which is the outline's
   ...
   head's outer face is |z| 0.7800, 107.5 mm outboard of the tyre's inner
   ...
   without anyone asking for it, because the head lives in the tyre's widest
   y band and the rail did not. It holds, and it holds by twelve millimetres.
   Anything that lengthens the sill forward, or any wheels-11 that moves the
   front tyre aft or inboard, spends that first.
   ...
   of end plates. So the string is not quantised at 210 and the PACK is.
   ...
   - wheels-10 carried in section: front tyre inner sidewall |z| 0.6725,
...
export const SYSTEM = {
  ...
  interfaces: {
    /* ── THE SILL, DECLARED ON BOTH ITS FACES, BECAUSE NEITHER MODULE CAN
       ...
       out into the shadow the tyre column already owns. body-11's floor pan
    ...
    /* The corner is one decision and two modules make it, so it is declared
       ...
       the tyre's inner sidewall and the two halves say opposite things about
       ...
       centre. wheels-10's BUILT sidewall measures 0.6725, because its
       ...
       plane and 17.5 mm inside the built tyre AT THAT STATION. Neither of
       those is this corner's clearance: measured surface to surface over
       the whole corner the pack comes within 10.73 mm of the tyre, on the
       ...
       the tolerance drops to 0.018 to keep the same realisation window.
       ...
       jobs in tools/interfaces.js: it is the realisation window, the
       ...
       there would be touching the tyre, so tol has to be at least the
       clearance the pack holds before the key can pass at all. That draft
       set it to 8 mm against a 7.5 mm clearance, half a millimetre of
       margin, which would have turned RED the first time a later rung cut
       the notch one millimetre deeper. 10 mm is wheels-10's own running
       ...
       reads: no vertex of this part may reach the tyre. That is a physical
       ...
       millimetres aft of here and the shield's edge flange 0.720, so
       ...
       front tyre's aft-most vertex within the pack's own height band is
    ...
    'front-corner-pack-outboard': {
      ...
      note: 'front tyre inner sidewall less wheels-10\'s 10.0 mm running clearance. This part stops at |z| 0.6550, 7.5 mm inside the plane, 17.5 mm inside the built tyre and 4.3 mm off the cell stack. The lid chamfer aft of x 1.1016 is outboard of the plane and clear of the tyre: see the note above.',
    },
  },
  parts: {
    bricks: {
    ...
    sill: {
      ...
      fail: [
        ...
        'THE FRONT CORNER GOT 18.1 mm TIGHTER AND NOBODY ASKED FOR IT. The head lives at y 0.3020 to 0.4200, which is the tyre\'s widest y band, and the rail it replaces did not. Swept about (1.45, 0.740) through 0 to 14 degrees of lock in the head\'s own band, the front corner reaches aft to x 1.0927 at 6 degrees against a head that ends at 1.0800: 12.7 mm in x, and x is the ONLY clearance there, because the head\'s outer face at |z| 0.7800 is 107.5 mm outboard of the tyre\'s inner sidewall. battery-10\'s rail had 30.8 mm in the same measurement. Any wheels-11 that moves the front tyre aft or inboard spends that first.',
    ...
    strikeshield: {
      ...
      fail: [
        ...
        'Titanium against an aluminium sill is a galvanic couple wherever moisture bridges them, the same problem the carbon lid solved with an isolation ply; the edge flanges carry a glass ply and a sealed fastener, and a damaged one starts slow attack on the rail flange rather than on the shield.',
    ...
    cornerclose: {
      ...
      specs: [
        ...
        ['Clearance', 'Pack to wheels-10, surface to surface: tyre 10.73 mm, disc 10.00, rim 21.35, caliper 33.06'],
...
/* ── THE GEN 10 NOTCH ──────────────────────────────────────────────────
   ...
   method, an exact plane slice clipped to a y band:
     ...
     0.6500  the cell stack. Layer faces, 36,677 of battery-7's 36,731
             ...
             within a nanometre, because 0.65 in float32 is 24 nanometres
     ...
     0.6725  wheels-10's built front tyre inner sidewall, a 135 mm section
             on the 0.740 centre. wheels-9's was 0.6625 on a 155 mm
             section, so the notch clears the wider of the two by 7.5 mm
             and the one this preset runs by 17.5.
     0.5940  ten millimetres inboard of wheels-10's brake disc inner face
   ...
   And the x stations:
     ...
     1.105   sized so the lead-in chamfer stays clear of the tyre shoulder,
             ...
             slice of the built tyre triangles at x 1.100, clipped to this
...
export function build() {
  ...
  /* THE STRING TAKE-OFF, REROUTED. battery-7 ran this cable outboard and
     ...
     the whole pack that sat outboard of the front tyre and was not lid,
     ...
     rail only once it is behind the tyre's plan circle, and lands on the
  ...
  for (const zs of [-1, 1]) {
    ...
    /* THERE IS NO SECOND BEAD, AND THAT IS A DELETION WITH A REASON RATHER
       ...
       a chamber. A sub-millimetre film between two bolted flanges is the
    ...
    if (zs > 0) {
      /* The 810 V return, carried from battery-10 without a coordinate
         ...
         web's inner wall every 300 mm, centred on y 0.2775 so the jacket
  ...
  for (const zs of [-1, 1]) {
    /* The side wall used to run to x 1.284 at |z| 0.686, so 204 mm of it
       stood inside the front tyre. It stops at the notch now and the
  ...
  for (const zs of [-1, 1]) {
    /* The straight run ends at x 1.045 rather than 1.190. battery-7's own
       ...
       front tyre, and the notch makes that unarguable: at |z| 0.665 the
  ...
  /* THE FRONT CROSSOVER IS NOW A FLATTENED DUCT, and this is the fix
     ...
     first millimetre clear of the fifth brick's end plate, and a short
...
/* ── Verification notes (measured off built geometry, not prose) ─────────
   ...
   to the centimetre", it "is not a keep-out box any more", and it "is
   ...
   0.4200 rounded outward to the centimetre in the convention SPEC.md
   ...
   realised: the outer face has vertices on |z| 0.7800 over the whole run and
   ...
   tools/interfaces.js realise() over both modules, sill-outboard reads
   ...
   checked separately, by ray parity along z through each chamber's centre.
   ...
   tyre, the strike shield skin has occupied |z| up to 0.700 over the same
   band since the module was written, the vapor headers 0.676, and a front
   tyre at 0.6625 and a pack at 0.700 cannot both be right. All four items
   ...
   the only guard on a new part fouling its own neighbours is somebody
   ...
   part fouls its neighbours the same way. Run over all 36 part pairs of
   ...
   identically on battery-7 and here", which is the one thing it was not:
     ...
     of |z| 0.6625, all of them inside the front tyre, and the notch could
