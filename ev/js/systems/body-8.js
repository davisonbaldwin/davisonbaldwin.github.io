/* Gen 8 body: the tail.
   ...
   wheels-7's rear tyre face at |z| 0.8325 and asks for it to be verified.
   It is wrong, in this module's favour. 0.8325 is 0.74 plus half of a 185
   ...
   vertex of wheels-7's built rear tyre, wheel and cover rotated about the
   ...
   liner face against the swept tyre where body-7 held 8.1. Overall length
   ...
   tyre: a shallow station is not a separation risk, and the pin is what
   makes it shallow.
   ...
   moves by less than one square centimetre, because the front wheel
   ...
   1.62 m front track and a 1.48 m rear the front and rear tyre bands no
   longer coincide, so their union is 225 mm per side against wheels-6's
   185, and the tyre columns go 0.1036 to 0.1260 m2. A lands at 2.121,
   ...
   is 5 square centimetres better than the body it replaces and 0.0219 m2
   worse than the number the ladder has been quoting. Almost all of that
   0.0219 is the 0.0224 of extra tyre column, a Gen 7 consequence that
   ...
   R20, rear centres |z| 0.74, faces 0.6625/0.8175 rear and 0.7325/0.8875
  ...
  parts: {
    ...
    'rear-casting': {
      ...
      fail: [
        ...
        'Yield the casting and the shell is done, unchanged from Gen 1, now with a 5.15 m body to re-index afterwards.',
      ],
      explode: [-0.85, -0.35, 0],
    },
    'crash-rails': {
    ...
    cage: {
      ...
      fail: [
        ...
        'The new C-pillar path is 80 mm shorter and lands on aluminium rather than into a bonded panel, so it is a fresh galvanic couple at a new place; it takes the same adhesive-and-rivet isolation the rest of the cage uses, and it is a new inspection point rather than a solved problem.',
    ...
    skin: {
      ...
      fail: [
        ...
        'The bonnet leading edge still runs at 43 degrees because thermal-7 inherits thermal-6\'s condenser inlet header at y 0.770, so the pedestrian head-impact map is body-7\'s and is still an extrapolated rather than a certified result.',
    ...
    'front-fairings': {
      ...
      specs: [
        ...
        ['Contained lock', '14.0 deg as drawn, a 21.4 m kerb circle; the 155 section allows 17.2 and ~16.9'],
      ...
      fail: [
        ...
        'Snow chains are geometrically impossible inside a closed fairing and there is no quick-release panel; the answer is winter tyres and the manual has to say so.',
        'A punctured front tyre cannot be inspected without removing the fairing lower closeout, so the roadside case is a pressure sensor and a tow rather than a visual check.',
        'The pod is now oversized for the tyre it encloses by 37 mm per side, so it carries frontal area it does not need and will keep carrying it until a body generation is scoped forward of the B-pillar.',
      ],
    ...
    'tail-cone': {
      ...
      fail: [
        ...
        'Boot volume is the visible bill and it goes the wrong way in both directions at once: at x -2.15, where a load floor would want to end, this tail is 1.498 m wide against body-7\'s 1.622 and its lower edge sits at y 0.426 against 0.380, so the well loses 124 mm of width and 46 mm of depth at the same station. This is a frunk and a shallow rear well, not an estate, and it is a smaller well than the one body-7 already apologised for.',
      ],
      explode: [-1.45, 0.30, 0],
    },
    'rear-arch-closeouts': {
      ...
      specs: [
        ...
        ['Lower lip', 'Turns inboard from the flank edge to 14 mm off the swept tyre'],
      ...
      fail: [
        ...
        'Stone impact from the tyre is continuous and directly normal to the surface: this is the one CFRP part on the car that sees gravel at 100 km/h from 45 mm away, and it takes an elastomeric sacrificial coat rather than paint.',
        'The 14 mm lower-lip clearance is against a swept envelope that assumes suspension-4 holds plus and minus 3 degrees; a rear-steer overtravel fault puts a moving tyre into a bonded lip, so the actuator end stop is now a body-protecting component and belongs on the shared FMEA.',
      ],
    ...
    canopy: {
      ...
      fail: [
        'One deep chip still condemns the band and the replacement must re-index to the loft within half a millimetre, carried from body-7 unimproved.',
        ...
        'This body and interior-6 still disagree by about 20 degrees of seatback at both rows, and the rear belt towers interior-6 draws stand outside this loft by 98 mm at (x -1.15, y 1.212) where they stood 107 mm outside body-7\'s; the tail improves an existing disagreement by nine millimetres, which is another way of saying it neither caused it nor fixes it, and an interior generation owes both.',
      ],
      explode: [0, 1.05, 0],
    },
    doors: {
      name: 'Flush doors and camera pods',
      tagline: 'Carried from body-7 without a millimetre moving, because every shutline on this car is forward of the closure.',
      mass: 34,
      count: 4,
      specs: [
    ...
    lightbands: {
      name: 'Light bands',
      tagline: 'Front band carried to the millimetre because autonomy-4 builds against it; the rear band is the same 1.00 m on a taller face.',
      mass: 4,
      count: 2,
      specs: [
    ...
    'diffuser-flap': {
      ...
      tagline: 'Eighty millimetres more ramp and eighty millimetres less exit span, and the arithmetic of which of those wins is not the one this module first wrote down.',
      mass: 5,
      specs: [
      ...
      fail: [
        ...
        'The departure angle is 15.9 degrees measured from the rear contact patch, against body-7\'s 16.0, so 100 mm of extra car cost a tenth of a degree because the longer ramp also rises further; the real parking bill is the 5.15 m length and a rear overhang of 1.325 m, which is a kerb-strike geometry on angled bays rather than a clearance number.',
        'Deep snow packs the expansion and the diffuser becomes a plough; rated for the load, parked at 13 by the controller, and still beyond the drag model\'s vocabulary, carried from body-7 with the same apology.',
      ],
      explode: [-1.0, -0.4, 0],
    },
    'arch-louvers': {
    ...
    'venturi-floor': {
      ...
      fail: [
        'Kerbs and ramps still rake the lip first; the shear-away fasteners cost a bracket, carried through five generations because car parks have not changed.',
        'A narrow splitter feeds a narrow stagnation region into the tunnels, so the front tunnel inlets are sensitive to yaw and the crosswind case is the one the tunnel programme should re-run first, unchanged and still outstanding from body-7.',
...
/* ── Geometry ──────────────────────────────────────────────────────────────
   ...
   - SWEPT REAR CORNER. wheels-7's rear tyre, wheel and cover vertices,
     ...
     read off the SWEEP table below as a continuum rather than at the tyre
     mesh's own vertices, which report a looser 22.5 mm because they land
     either side of the tight station
     (body-7 held 8.1 mm against a 185 tyre on a 1.62 m track).
   ...
   - FRONTAL AREA, integrated exactly as body-7 integrates it and on the
     ...
     m2. Lower nose band 0.173 carried. Tyre columns 0.1260 on wheels-7's
...
const STATIONS = [
  ...
  /* 2  bonnet, frunk zone                            body-7, unchanged */
  ...
  /* 7  windscreen header station                     body-7, unchanged */
...
/* Flat end cap built from a ring's own outline, so the nose and Kamm faces
   ...
   of a curved edge. dx pushes the centre of the cap behind the outline.
...
/* ── The crisp shell: one master grid ──────────────────────────────────
   ...
   smoothed. This module's clearances are quoted to the millimetre off built
   geometry: 18.9 mm at the liner face over the swept rear tyre, 43 mm at
   the nose to thermal-7's inlet header, 6 mm from the arch hoop to the
   flank. A surface that moves three centimetres between stations
   ...
   every station from the windscreen header aft and a feature that persists
...
/* ── The louver recesses ────────────────────────────────────────────────
   ...
   WHAT THIRTY MILLIMETRES BUYS, and the arithmetic that says otherwise is
...
/* ── The declared hard lines ────────────────────────────────────────────
   ...
   windscreen header to the Kamm face, and forward of the cowl it turns
   ...
   wrong without it. The bonnet side gaps run from ring fraction 3.7 and
...
for (const g of SHELL.grooves) {
  ...
  /* A run-out is an INTERVAL, not two lines. Declaring only the two ends
     ...
     pocket's aft fade runs from row 4.8456 to 4.8826 and the bonnet rear
...
/* The marque: a chevron, extruded 2.4 mm with a bevelled rim. A decal has
...
function buildShell(sys) {
  /* ── skin: nose flank, the fixed panels either side of the door aperture,
     the bonnet band and the nose face. Everything here is body-7's surface,
  ...
  /* ── rear wheelhouse closeouts: the part wheels-7 filed a defect for.
     A hoop swept at r 0.400 about the axle from just ahead of the tyre,
     ...
     flank's bottom edge in to 14 mm off the swept tyre. Nothing below the
  ...
  /* Strake fixings, two per strake, taking the ramp's own rotation so the
     ...
     coordinates matter and a first pass had both wrong. The strake's centre
  ...
  /* ── arch louvers, in the recess cut for them above. The blades keep
     ...
     widest the body is at its height. Each bank is centred in its own bay
  ...
  /* Bank fixings, one through each frame. A louver bank is a serviceable
     ...
     direction. The seat is taken from the frame's own face centre and the
  ...
  /* front face SET behind the valance along its own normal, then back off
     half the lattice depth to get the group centre */
