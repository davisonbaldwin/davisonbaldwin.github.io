  ...
  parts: {
    ...
    displays: {
      ...
      fail: [
        ...
        'Continuous health disclosure teaches its own complacency: a driver who has seen two lanes for a month stops reading the band, which is why any restriction that changes stopping behavior also changes the drive mode, not just a color.',
    ...
    console: {
      name: 'Center console and comfort supply',
      ...
      fail: [
        ...
        'A leak anywhere in the seat circuit shows up here as a pump that never stops cycling, which the current telemetry catches long before an occupant notices a soft bolster, and which nothing else in the cabin can localize.',
      ],
      explode: [0.15, 0.6, 0],
    },
    pedals: {
...
/* ── Geometry ──
   ...
   for that. Driver sits at z -0.38. Hard points honored: battery lid
...
const DZ = -0.38;                 // driver centerline
...
export function build() {
  ...
  const centerBezel = lib.box(0.022, 0.175, 0.35, M.plastic);
  const centerFace = lib.box(0.006, 0.165, 0.34, M.sensor);
  centerFace.position.x = -0.013;
  center.add(centerBezel, centerFace);
  center.position.set(1.020, 0.968, 0);
  center.rotation.set(0, -0.10, -0.14);
  displays.add(center);
  ...
  /* THE CUSHION IS MOLDED AROUND THE COMPUTE INSTEAD OF THROUGH IT.
  ...
  /* Two outboard bolsters at the OLD full section, then the center band split
