/* Wheels and brakes, Gen 14: the corner does not move, and both axles are
  ...
  parts: {
    ...
    'front-pants': {
      ...
      specs: [
        ...
        ['Band', 'r 0.373 inner (18 mm over the tread at the crown), 61 degrees behind the top to 137 ahead; |z| 0.6725 to 0.8255 at the crown, the inboard edge at the tire face to 42 degrees, 0.700 by 50, then 0.665 on the nose and 0.715 on the tail from 58'],
        ...
        ['Tightest', '12 mm, leg to the flank at 14 degrees of lock (carried); 14 mm, the crown edge to the flank at 42 degrees off the top at lock; 18 mm, band to tread'],
    ...
    'rear-saddles': {
      ...
      specs: [
        ...
        ['Band', 'r 0.373 inner about the rear axle, 4 mm carbon sandwich, |z| 0.6725 to 0.8075 (the tire\'s own section, no lip), from 65 degrees ahead of the top (x -1.108, y 0.514) to 75 behind (x -1.814, y 0.452)'],
        ...
        ['Tightest', '13.5 mm, band outer edge to the spat wall at 3 degrees of rear steer; 17 mm, riser to the flank; 19 mm, jog to the tread shoulder'],
...
const PANT = {
  ...
  ZIN_TAIL: 0.715, // band inboard edge on the tail, from 58 deg down: wheels-13's 0.690 put the tail tip 3.6 mm from hv-11's spine at full lock and full rebound, swept about the axle plane; 0.715 puts it 17 mm clear
...
const SADDLE = {
  ...
  ZOUT: 0.8075,    // outboard edge: the tire's section maximum plane, no lip
  ZIN_NOSE: 0.695, // inboard edge from 45 to 65 degrees ahead: autonomy-13's compute corner at (-1.142, 0.552, 0.656) under 3 deg of rear steer
  TH: 0.004,       // laminate
...
function rearSaddlePart(s) {
  ...
  const zin = (deg) => {
    if (deg <= 45) return SADDLE.ZIN;
    const t = Math.min(1, (deg - 45) / 10);
  ...
  for (let deg = -SADDLE.ARC_TAIL; deg <= SADDLE.ARC_NOSE + 1e-9; deg += 10) {
    ...
    const z0 = zin(deg), z1 = SADDLE.ZOUT;
  ...
  for (const deg of [-SADDLE.ARC_TAIL, SADDLE.ARC_NOSE]) {
    const th = (deg * Math.PI) / 180;
    const z0 = zin(deg), zw = SADDLE.ZOUT - z0;
