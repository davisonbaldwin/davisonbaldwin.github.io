...
export const SYSTEM = {
  ...
  parts: {
    condenser: {
      ...
      mass: 7,
      specs: [
        ['Core', '770 x 300 x 45 mm brazed aluminum, 0.23 m² frontal'],
    ...
    radiator: {
      ...
      mass: 8,
      specs: [
        ['Core', '500 x 140 x 40 mm louvered, lower row of the stack'],
      ...
      why: 'A radiator is sized by the hour it cannot refuse, and this generation moved that hour. Once the pack rejects through its own condenser, nothing left on the glycol side has a parked worst case worth 28 kW, so the core shrinks by two thirds and the mass goes into fans that serve both rows. Keeping the fans in one bank behind both cores is the packaging argument Gen 1 made and it survives: the frontal plane is the scarce resource in the nose, and every square centimeter spent on a second shroud is area not spent on heat transfer.',
      fail: [
        'One fan out is now a bigger event than it was: with 25 kW of condensing duty on the same bank, the survivor at full speed holds roughly 60 percent of design air, and the pressure set answers by raising saturation temperature, which the pack pays for in aging rather than in minutes.',
    ...
    'glycol-loop': {
      name: 'Glycol circuit',
      tagline: 'What is left of Gen 1\'s ring main after the pack stopped drinking: 3.4 liters, two pumps, six ports instead of fourteen modes.',
      mass: 7,
      specs: [
      ...
      fail: [
        ...
        'Corrosion inhibitors deplete on a clock rather than on mileage, and a circuit this small has proportionally less inhibitor per meter of aluminum than the one it replaced, so the fluid service interval got shorter even though the fluid does less work.',
      ],
...
const CY = 0.59;                // condenser core center y: 0.44 to 0.74
const GY = 0.33;                // radiator core center y: 0.26 to 0.40
const RCV = [2.14, 0.375, -0.40];   // receiver axis center
